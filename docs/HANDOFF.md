...
> 2. **`/board` — ✅ BUILT (slice 5):** the rush cut (`boardColumnFit` + a measured cap, `+N more`),
>    the stale tell, the band at TV scale, the sound TOGGLE with a said refusal, the dictionary
>    sign-in + `BRAND_NAME`, the hairline + anti-burn drift, the shelf-wait ceiling (`shelfWait`),
>    the identity group, the cap on `.orb-table-up` + the opacity flash. board-10 retired (P6f closed).
...
> 12 components, 1 in `packages/db`) · **1787 qr + 142 ui tests at the time (2788 + 146 today)** ·
...
> ### Counts on this head, measured not transcribed
>
> **334 mutants at the time (741 today)**, **1372 qr + 138 ui tests at the time (2788 + 146 today)**, 69 target modules at the time (111 under `apps/qr/lib` today, 127 in all), 97 local
...
>
> **Gate today:** 741 `verify:slice` mutants green · `pnpm check:docs` clean (100 files, 2788 qr tests + 146 ui tests) · CI green · then the two reviewers.
>
