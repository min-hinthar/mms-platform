...
> sentences) — await Min's native check** (listed on K15). The full 1108-mutant `verify:slice` run
...
>
> **Then a blind review (money semantics · concurrency · a11y) returned 3× REJECT; every finding was
> fixed or rejected with evidence in ONE fix pass** — `p2c/pad2` (P1–P13) and `p2c/reg2` (R1–R7),
> merged `8c44651` · `a5a9120`, the "Blind review" block of the CHANGELOG entry. Closed P2cd; P2ch ·
...
>
> **Gate today:** 1113 `verify:slice` mutants · 163 target modules (138 `apps/qr/lib`, 3 API routes,
...
> (`p2a/*`: send · padserver · tablet · register) merged here. Its CHANGELOG entry is the as-built
> and DESIGN-LANGUAGE §17 carries the new rules. P2k is closed (the console Send + Undo on the table
> page); the rows 2a found are **P2v–P2ap** in OPEN-ITEMS, and `supabase/tests/staff_fire_undo_test.sql`
> has never run (CI-only — watch its first run and do the red-first step there, P2ac).
> **Owner decisions (2026-09-24):** counter orders will be sendable before payment with an "Unpaid —
> collect at pickup" flag (2f, needs a migration — until then the console refuses a counter send and
> says "The kitchen starts this order when it’s paid."); the staff language becomes a three-way
> Burmese / Both / English per-device setting in Profile (2e).
> **The Phase 2 plan:** 2a built · **next 2b** kitchen tickets + feedback · **2c** the order pad +
> register settle + the settle gate (send-kitchen commit B; the pad removes 2a's interim "Review · N
> not sent →" bridge on the add page) · **2d** the floor + the tablet split + the counter bell (the
> floor's "not sent" signal, `staffOwedSendUnits`, ships pure in 2a) · **2e** language · **2f**
> counter cook-before-paid. Then Phase 3 (photos, live Stripe keys, RUM).

> ## ⏭️ NEXT SESSION — start here (2026-09-24 · Phase 1c — the add, the removal, the card form, the Stars, the market's front door — on branch `claude/inspiring-cori-4rf37k`)
>
> **Where the owner's polish ask stands:** Phase 0 (#299), 1a (#300) and 1b (#301) are merged; 1c is
> this branch's PR. Its CHANGELOG entry is the as-built and DESIGN-LANGUAGE §23–§27 are the rules.
> 1c was built as five parallel worktree branches (`p1c/*`, one spec each) merged here — the only
...
>
> 1. **The sheet primitive (`packages/ui`) — ✅ BUILT (slice 4, draft PR #295):** manager-9
>    (`Sheet.closeLabel {idle, busy}` as sr-only DOM text, `sheetCloseLabel(lang)` for the five
>    staff callers), manager-10 / M76 (NOT the `forceMount` + `AnimatePresence` plan the audit
>    sketched — a CSS exit pair on `[data-state="closed"]` that Radix's own `Presence` honours, once
>    `SheetContent` became the portal's child and forwarded its ref through the provider, and once
>    the three parents that unmounted their sheet on close held the subject through the exit with
>    `useSheetSubject`; LEARNINGS #129), and K29(b)'s cash-settle confirm → `Sheet` (`busy` as a
>    transition — the M82 caller guard caught the hand-rolled boolean; a refusal stays in the sheet;
>    a landed settle unmounts it and the trigger reads busy; LEARNINGS #130 on the focus-restore
>    timer). The blind pass (REJECT: a table's settle parked the cashier in a busy modal) is closed
>    on the PR. One device check is owed and filed with M76: the drag-release exit's first frames.
> 2. **`/board` — ✅ MERGED (#296, slice 5):** the rush cut (`boardColumnFit` + a measured cap on a
>    `flex: 1` list of one-line rows, `+N more`), the stale tell, the band at TV scale, the sound
>    TOGGLE with a said refusal and a one-arm guard, the dictionary sign-in + `BRAND_NAME`, the
>    hairline (the drift withdrawn → K36), the shelf-wait ceiling (`shelfWait`), the identity group,
>    the cap on `.orb-table-up` + the opacity flash behind the text. board-10 retired (P6f closed).
> 3. **Menu · Tips · Glossary — ✅ BUILT (slice 6):** §17 on the price editor (the last four native
>    sites on the screen), the sr-only region + the row echo, confirmed values before the refresh,
...
> refusal path dropped `refresh()` — and with it the `outcome === "failed"` arm that is the only
> thing which gets a diner off a cart the register has already settled. A closed cart makes the
> write AND the diagnosis read throw, so the "better" path was strictly worse than the swallow it
> replaced. **When you replace a catch-all with something more precise, enumerate what the
> catch-all was also doing.** Two further retractions landed with it: "every throw in `setQty`
> precedes the RPC" was false, and a comment asserting React's batching as the reason the banner
> wins the slot was a mechanism invented to explain a true observation.
>
...
> `lib/served-today.ts`, a second view of the rail the board already had, under a `.staff-seg`
> track; three honest states, the failed read SAID rather than shown as an empty day); the ONE
> **service-day rule** (`lib/day-window.ts` · `dayStartIso(now, tz)` from `pickup_config.tz`, the
> same zone `mms_kds_stats` derives "Avg today" from — `laDayStartIso` is now that rule applied to
> LA, and its rolling-24h mutant moved with it); the ONE **Burmese name loader** (`lib/line-names.ts`
> — kitchen, expo, `/api/board` and the served rail read it; F18 (a)); and the wall's **wait
> minutes** (K32 (a) — `readyMinutes` off the DB clock in `/api/board`, drawn by `BoardCard`, plus
> the shared `queueFloorIso` and a 503 on a saturated 60-row read instead of an empty wall). Two
> new mutants on the board route, one retargeted; `served-today` and `line-names` are pinned by
> value tests watched red by hand (neither is a money module).
>
> **A4·2 — Tables & settle** is built too (the second commit on `a4-local`, its own PR after A4·1's):
> the counter's ONE screen in the order the counter person works it — the Start zone, the tables and
> the open counter orders in one `role="list"` (the counter read rides the floor's own snapshot:
> `lib/register-queue.ts` · `readRegisterQueue` inside `getFloorView`; `lib/floor-rows.ts` ·
> `mergeFloorRows` states once where the two lists meet), the to-go lane with **K30 (B)**
> (`lib/expo-rules.ts` · `kitchenStateOf` + `compareExpoTickets`, advisory, a **Kitchen done**
> badge), today's takings (`DayCash`). `/staff/register` and `/staff/expo` are redirects to
> `/staff?floor=1`; `check-staff-lang` rule 4 exempts a redirect-only page by parsing it (14/14 + 2
> exempt); the counter's help sheet is six cards (the bump card composed from the takeaway board's two bump sentences, the paper card moved whole)
> and the board's own door is gone. Nine mutants, all watched red; `expo.ts` joined the mutate set
> with a wiring suite. Both A4 commits were built on `9744800` (A3 BEFORE its Codex fixes); each is
...
>
> **1113 `verify:slice` mutants** · **163 target modules** (138 under `apps/qr/lib`, 3 API routes,
> 21 components, 1 in `packages/db`) · **1787 qr + 142 ui tests at the time (3997 + 228 today)** ·
...
> the mutant total and the `apps/qr/lib` bucket and nothing else on this line. `CLAUDE.md` names this
> exact shape "an operational hazard, not a typo": this enumeration is the operator's only list of
> files that may be sitting on disk as deliberately-broken mutants after a killed run. MEASURE the
> buckets — the command is in `CLAUDE.md` beside `verify:slice` — never carry them by hand.
>
...
> ### Counts on this head, measured not transcribed
>
> **334 mutants at the time (1113 today)**, **1372 qr + 138 ui tests at the time (3997 + 228 today)**, 69 target modules at the time (138 under `apps/qr/lib` today, 163 in all), 97 local
...
>
> **Gate today:** 1113 `verify:slice` mutants green · `pnpm check:docs` clean (100 files, 3997 qr tests + 228 ui tests) · CI green · then the two reviewers.
>
...
> queue, catalog-cache "≈$" estimates. 88 mutants at the time (1113 today) — and
