...
> ## ⏭️ NEXT SESSION — start here (2026-09-13 · A3 merged as `1058087` (#280); A4·1 is PR #281 — Codex rounds 1 and 2 fixed (`3ceed2c`, the round-2 commit); the per-head round is the last gate; A4·2 and A4·3 are staged behind it)
>
...
> ### What is next (A4)
>
> 1. **A4·2 — Tables & settle**: floor + register + expo on one screen, the old routes as
>    redirects, K30 (B)'s "kitchen done" badge on the to-go lane. Then A4·3 (the manager rails and
>    M204's receipt-read refund console — `refunds.ts` has no mutant, land one), A4·4 (Sign-in as one
...
>    (`kds.served.*` — `kds.served.moreUnknown` and `kds.served.voided` since the Codex rounds — `kds.a11y.served`, `kds.a11y.railView`, `board.card.*`) is a machine draft.
>
> ---
>
...
>
> **646 `verify:slice` mutants** · **113 target modules** (100 under `apps/qr/lib`, 3 API routes,
...
> ### Counts on this head, measured not transcribed
>
> **334 mutants at the time (646 today)**, **1372 qr + 138 ui tests at the time (2432 + 142 today)**, 69 target modules at the time (100 under `apps/qr/lib` today, 113 in all), 97 local
...
>
> **Gate today:** 646 `verify:slice` mutants green · `pnpm check:docs` clean (99 files, 2432 qr tests + 142 ui tests) · CI green · then the two reviewers.
>
...
> queue, catalog-cache "≈$" estimates. 88 mutants at the time (646 today) — and
