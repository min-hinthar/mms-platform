...
| Q10 | low      | Line-merge dup key uses modifier **labels** not option ids + ignores `unit_price_cents` → same-name options with different price deltas fold to the wrong price (menu-data-dependent).                                                                                                                                                                                                                                                                                                                                                                            | Add `unit_price_cents` (and ideally option-id set) to the dup-match predicate. **M3 widened the blast radius**: a label-collision merge now also stamps one unit's `modifier_option_ids` onto the whole merged qty, and reorder replays the wrong option SILENTLY (all stored ids honor → no disclosure) — the fold key moving to ids fixes both. | 📋                                |
