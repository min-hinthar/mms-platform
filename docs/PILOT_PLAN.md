...
| **P2** | ✅ **DONE (2026-09-05) — both PRs. PR A `5715781` (#259) shipped the spine; PR B is #260.** The staff-DEVICE locale (`mms_staff_lang`, default MY, `path:"/"` so the wall TV is not starved) with an UNGATED writer, and a dictionary that is now **469 keys** — PR B added 336 across `floor.* · table.* · reg.* · settle.* · browse.* · expo.*`. Every one of the fifteen staff pages reaches the language control; every accessible name on 63 staff files comes from `lib/staff-labels.ts`; the outage sentence is bilingual at 46 arms through `<OutageText>` at the render site rather than by threading `lang` through an auth path. Closed on the way: **P2c** (all three parts), **P2g** (a table card announced _"settling"_ while its chip read _"Splitting"_ — a WCAG 2.5.3 mismatch in ENGLISH, live since S1.2) and **P2h** (the outage shell, which now speaks the device language AND mounts the control — the one screen where `setStaffLang`'s ungated design is the whole point). ⚠️ **Every MY value is a Claude-authored draft pending Min's native check (K15)** — which is what P5's printed glossary turns from a blocker into pilot output. Was: **Staff chrome dictionary + device language.**                                                                                                                                                                                                                                                                                    | 2 PRs          |
