...
| **P6** | ✅ **BUILT (2026-09-05).** `/board` keeps its customer-safe Ready column and gains the kitchen pulse band: live ticket count · the oldest ticket's age in minutes · an unattributed all-day rail · dine-in as **table number + `cooking`/`Food up`**. The boundary is `lib/board-pulse.ts`'s output type (no field for a name, an id, a per-table dish list, a modifier or an amount), the table strip is a positive allowlist mirroring the orders half, and the rail is **withheld below three live tickets** — a bound on the worst SINGLE-FRAME reading, stated as that and not as anonymity (the open channels are **P6a**). Three things the blind recon pass changed: the word is **`up`, not "Ready"** (nothing records that a plate reached a table, and "Ready" on a dining-room wall instructs a guest who has nothing to do); **ghost sessions** are off the strip (`is_member` needs `expires_at > now`, so past the TTL the diners cannot act on their own cart — nothing closes it and nothing extends it); and `oldestFiredAt` became **`oldestMinutes`**, because an exact fire stamp beside one table states that party's order instant to the room. Kitchen reads degrade to `pulse: null`, never a zeroed band. Residuals filed: **P6a–P6g**.                                                                                                                                                                                                                                           | 1 PR           |
