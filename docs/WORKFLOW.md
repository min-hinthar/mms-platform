...
1. **`pnpm verify:slice` FIRST** — the mechanical gate. Runs the full gate, applies 228 semantic mutations to the money/authority modules (each **must** turn its owning suite red), and mirrors CI's orphan check. A few minutes, zero tokens. A **surviving** mutant means the fixture is degenerate; a **stale** one is a failure, not a skip.
