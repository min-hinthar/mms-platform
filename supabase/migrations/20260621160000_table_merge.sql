...
-- re-parent — it never trusts or recomputes a client amount. The caller (lib/floor.ts mergeTables, after
-- requireStaff) guarantees both sessions share a MODE, so the stored per-line tax basis (dine-in vs to-go)
-- stays correct across the move.
--
