...
-- INVOKER (the only caller is a service-role server action that has already authz'd the opener — staff via
-- requireStaff OR the diner via assertCartMember; T3). Row-locks the cart so a concurrent settle/clear loses.
