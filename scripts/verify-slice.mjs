...
const MUTANTS = [
  ...
  {
    id: "refusal/every-refusal-reads-as-a-session-failure",
    file: "apps/qr/lib/cart-freeze.ts",
    suite: "lib/cart-freeze.test.ts",
    why: 'T14 \u2014 THIS IS THE SHIPPED DEFECT, restored. `TableCartProvider`\'s `add`/`setItemQty` catches flashed "Reconnecting to your table\u2026" and re-minted the table session for EVERY throw, while `add`\'s own comment listed "a refused write (cart locked, a stale/invalid modifier selection)" among the causes. A diner whose tablemate was checking out was told their connection had dropped and watched a session re-mint they did not need \u2014 the M116 fabricated-diagnosis class, surviving on /menu because Next redacts Server Action messages, so the server\'s own sentence never reaches the browser',
    find: '  if (!reread.ok) return { cause: "session" };',
    replace: '  if (!reread.ok) return { cause: "session" };\n  return { cause: "session" };',
  },
  {
    id: "refusal/settling-outranks-the-lock",
    file: "apps/qr/lib/cart-freeze.ts",
    suite: "lib/cart-freeze.test.ts",
    why: "T14 \u2014 the arms must mirror the server's own order. `addItem` and `setQty` each test `locked` first and `settling` second, so a cart that is BOTH was refused as locked; naming the settle there reports a reason the server never acted on, and points the diner at a split they may not be part of",
    find: '  if (freezeBlocksEdits(freeze) && freeze !== null) return { cause: "frozen", freeze };\n  if (reread.settling) return { cause: "settling" };',
    replace:
  ...
  },
  {
    ...
    file: "apps/qr/lib/cart-freeze.ts",
    suite: "lib/cart-freeze.test.ts",
    why: "T14 \u2014 the OVER-blocking direction, and it is not only copy: `refusalIsFreeze` runs as the PRE-write gate in `TableCartProvider`, so a refusal that manufactures a freeze from an editable cart blocks every tap the server would have accepted. Same shape as the delivery app's `computeDeliveryGate`, where a bare `!isOpen` folded into a submit gate disabled an entire valid ordering window with no escape",
    find: '  return { cause: "unknown" };',
    replace: '  return { cause: "frozen", freeze: "held" };',
  },
  {
    id: "refusal/gate-blocks-what-it-cannot-explain",
    file: "apps/qr/lib/cart-freeze.ts",
    suite: "lib/cart-freeze.test.ts",
    why: "T14 \u2014 the same over-blocking, one function over. `unknown` establishes nothing and `session` establishes an unreachable cart; treating either as a freeze turns the pre-write gate into a refusal of writes the server accepts, which is exactly the direction this repo has paid for before",
    find: '  return refusal.cause === "frozen" || refusal.cause === "settling";',
    replace: '  return refusal.cause !== "session";',
  },
  {
    id: "refusal/lock-sentence-forks-from-freezeNotice",
    file: "apps/qr/lib/cart-freeze.ts",
    suite: "lib/cart-freeze.test.ts",
    why: 'T14 \u2014 the "name it ONCE" rule applied to copy. The lock has FOUR sentences (peer, self-with-token, self-without, held) and `freezeNotice` picks between them; a hand-written string here makes /menu describe the same lock differently from the review step, and silently drops the distinction between a self lock that can be reopened and one that cannot',
    find: '    case "frozen":\n      return freezeNotice(refusal.freeze, peerName, false);',
    replace:
      '    case "frozen":\n      return "The order\u2019s locked while a checkout finishes.";',
  },
  {
    id: "refusal/unknown-borrows-the-reconnect-sentence",
    file: "apps/qr/lib/cart-freeze.ts",
    suite: "lib/cart-freeze.test.ts",
    why: 'T14 \u2014 the fabrication itself, isolated to the copy. "Reconnecting" is established ONLY by a re-read that failed, and it is the sentence that accompanies a session re-mint. Saying it for a write refused on a reachable cart tells the diner their connection is at fault and invites them to wait for a recovery that is not running',
    find: '      // Established: the write did not land, and the view beside this sentence is server truth (the\n      // caller applied the same re-read). Nothing about a session, a lock, or a connection.\n      return "That didn\u2019t go through \u2014 the order below is up to date.";',
    replace: '      return "Reconnecting to your table\u2026";',
  },
  {
    id: "refusal/settle-sentence-forks-from-the-constant",
    file: "apps/qr/lib/cart-freeze.ts",
    suite: "lib/cart-freeze.test.ts",
    why: "T14 \u2014 `SETTLING_NOTICE` is exported because `TableCartProvider`'s settle-TRANSITION effect renders the same string. Two copies of one sentence is how a copy edit leaves the refusal and the announcement describing one freeze two different ways, on the same screen, seconds apart",
    find: '    case "settling":\n      return SETTLING_NOTICE;',
    replace: '    case "settling":\n      return "The order is locked.";',
  },
  {
