...
const MUTANTS = [
  ...
  // The cash moment (lib/register-math), the settleCash compare-and-swap, and the settle components
  // that wire them. TerminalSettle.tsx JOINS the mutate set here (its first mutants).
  {
    id: "p2c-register/ladder-ceil-offers-exact-twice",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · register — a quick-cash note is the next multiple ABOVE what is due. `ceil` offers the due itself on a whole-dollar total ($20.00 → $20 beside Exact): two chips for one amount and one fewer real round-up",
    find: "      const c = (Math.floor(floor / d) + 1) * d;\n",
    replace: "      const c = Math.ceil(floor / d) * d;\n",
  },
  {
    id: "p2c-register/ladder-no-dedupe",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · register — several notes round to the same figure ($4.25 → $5 by the $1 AND the $5 step). Without the dedupe the row offers $5 twice and drops the $20 a guest actually hands over",
    find: "      if (!notes.includes(c)) notes.push(c);\n",
    replace: "      notes.push(c);\n",
  },
  {
    id: "p2c-register/ladder-offers-whole-dollar-plus-one",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · register — on a whole-dollar total the $1 step is skipped: nobody hands over $21 for $20.00. Without the skip the first round-up is a note that never crosses the counter",
    find: "      if (d === ladder[0] && floor % d === 0) continue;\n",
    replace: "",
  },
  {
    id: "p2c-register/ladder-unsorted",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · register — a ladder whose steps do not divide one another builds its notes out of order; unsorted, the chips read $9 · $7 · $14 and the cashier's eye has to hunt",
    find: "  return notes.sort((a, b) => a - b);\n",
    replace: "  return notes;\n",
  },
  {
    id: "p2c-register/tender-arms-swapped",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · register — over the due is CHANGE, under it is SHORT. Swapped, a $20 for $13.47 reads short and blocks the settle, and a $13 reads change $0 — the drawer comes up short with nothing on screen",
    find: "  if (tenderedCents > dueCents)\n",
    replace: "  if (tenderedCents < dueCents)\n",
  },
  {
    id: "p2c-register/tender-zero-reads-short",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · register — the tender is OPTIONAL (owner decision): an empty or zero field says nothing. Without the ≤0 guard a typed 0 reads 'Short $13.47' and blocks every settle the cashier did not type a tender for",
    find: '  if (tenderedCents == null || !Number.isSafeInteger(tenderedCents) || tenderedCents <= 0)\n    return { kind: "none" };\n',
    replace:
      '  if (tenderedCents == null || !Number.isSafeInteger(tenderedCents))\n    return { kind: "none" };\n',
  },
  {
    id: "p2c-register/settle-blocked-ignores-short",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · register — cashSettleBlocked is the ONE binding Settle reads (§22). Ignoring `short`, Settle stays live beside 'Short $2.10' and records a sale the drawer is short on",
    find: '  if (tender.kind === "short") return "short";\n',
    replace: "",
  },
  {
    id: "p2c-register/settle-blocked-ignores-the-tip-cap",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · register — the cash tip's ceiling is named ONCE (lib/tip `tipWithinAmountCap`). Dropped from the binding, a $1,000.01 tip leaves Settle live and the server refuses at the last tap",
    find: '  if (tipCents == null || !tipWithinAmountCap(tipCents)) return "tipCap";\n',
    replace: '  if (tipCents == null) return "tipCap";\n',
  },
  {
    id: "p2c-register/keep-change-offered-when-exact",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · register — 'Keep the change as tip' is offered only while change is OWED. With `>=` it stays on screen after the tap (the readout already says 'Exact — no change'), inviting a second tap on nothing",
    find: "  if (!(keep > tipCents)) return null;\n",
    replace: "  if (!(keep >= tipCents)) return null;\n",
  },
  {
    id: "p2c-register/handoff-change-on-a-short-tender",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · register — a short tender's paid card says what is STILL TO COLLECT. Without the branch it reads 'Change $0.00' while the guest still owes the difference",
    find: '  if (tender.kind === "short") rows.push({ k: "collect", cents: tender.shortCents });\n',
    replace: '  if (false) rows.push({ k: "collect", cents: 0 });\n',
  },
  {
    id: "p2c-register/cas-compare-deleted",
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/settle-cash-cas.test.ts",
    why: "Phase 2c · register — the compare-and-swap. Without it a settle whose total moved since the cashier read it records the NEW total: they collected $X, the ledger books $Y, and nothing says so",
    find: "    if (quotedCents !== undefined && quotedCents !== totals.totalCents)\n",
    replace: "    if (false)\n",
  },
  {
    id: "p2c-register/cas-low-quote-passes",
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/settle-cash-cas.test.ts",
    why: "Phase 2c · register — ANY difference refuses. With `>`, a quote BELOW the live total passes: the cashier collected less than the ledger records",
    find: "    if (quotedCents !== undefined && quotedCents !== totals.totalCents)\n",
    replace: "    if (quotedCents !== undefined && quotedCents > totals.totalCents)\n",
  },
  {
    id: "p2c-register/cas-compares-all-in-total",
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/settle-cash-cas.test.ts",
    why: "Phase 2c · register — the quote is the PRE-tip total (the figure `detail.settleTotalCents` shows). Compared against total + tip, every tipped settle is refused as 'moved' forever",
    find: "    if (quotedCents !== undefined && quotedCents !== totals.totalCents)\n",
    replace: "    if (quotedCents !== undefined && quotedCents !== totals.totalCents + tipCents)\n",
  },
  {
    id: "p2c-register/cas-refusal-outside-the-freeze",
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/settle-cash-cas.test.ts",
  ...
  },
  {
    ...
    file: "apps/qr/components/staff/CashSettleButton.tsx",
    suite: "components/staff/CashSettleButton.test.tsx",
    why: "Phase 2c · register (P2ab) — a REJECTED settle may have landed (the response was lost after the RPC committed). Said as a refusal, the cashier is told nothing was recorded and takes the money again",
    find: '          setError({ kind: "unknown" });\n',
    replace: '          setError({ kind: "server", text: String(e) });\n',
  },
  {
    id: "p2c-register/table-tender-hands-no-card",
    file: "apps/qr/components/staff/CashSettleButton.tsx",
    suite: "components/staff/CashSettleButton.test.tsx",
    why: "Phase 2c · register (owner decision 7) — a table that paid with a tender entered gets the rows-only paid card. Handing it up only on a counter order, the change figure vanishes at the tap — the one fact the cashier still needs",
    find: "        if (handoff || tenderAtTap != null) {\n",
    replace: "        if (handoff) {\n",
  },
  {
    id: "p2c-register/secure-close-no-page-reread",
    file: "apps/qr/components/staff/CloseSecureTabButton.tsx",
    suite: "components/staff/CloseSecureTabButton.test.tsx",
  ...
  },
  {
    ...
    why: "Phase 2c · register (P2r) — the reader panel's status is SAID through the page's one polite region. Not handed up, a screen-reader cashier hears nothing while the reader waits, fails, or loses the processor",
    find: "    onStatus?.(JSON.parse(spokenKey) as ReaderStatus);\n",
    replace: "",
  },
  {
    id: "p2c-register/reader-never-goes-blind",
    file: "apps/qr/components/staff/TerminalSettle.tsx",
    suite: "components/staff/TerminalSettle.test.tsx",
    why: "Phase 2c · register — after three missed polls the panel says it cannot see the processor. Never blind, it keeps saying 'waiting' over a reader nobody is watching, and the cashier takes a second tender",
    find: "  const blind = pollMisses >= BLIND_AFTER_MISSES;\n",
    replace: "  const blind = false;\n",
  },
  {
    id: "p2c-register/reader-landed-no-page-reread",
    file: "apps/qr/components/staff/TerminalSettle.tsx",
    suite: "components/staff/TerminalSettle.test.tsx",
