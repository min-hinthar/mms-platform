...
const MUTANTS = [
  ...
  },
  {
    ...
    replace:
      "            setQuote({ cents: quoted, basis, raisedAt: readsStarted?.() ?? readTicket });\n",
  },
  {
  ...
  {
    id: "staff-send-view/unsent-tab-close-never-offers-remove",
    file: "apps/qr/lib/staff-send-view.ts",
    suite: "lib/staff-send-view.test.ts",
    why: "Phase 2c · gate — the running-bill close is where the guest may have left, so its sentence offers removing the dishes beside sending them. Ignored, a card-on-file close over dishes nobody will eat is told only to send them — cooked for an empty table and charged to a guest who left",
    find: "  const k = running\n",
    replace: "  const k = false\n",
  },
  {
    id: "staff-send-view/unsent-line-retired-by-an-older-read",
    file: "apps/qr/lib/staff-send-view.ts",
    suite: "lib/staff-send-view.test.ts",
    why: "Phase 2c · gate — a raced server refusal is newer than a poll already in the air, which cannot see the dish that landed. Retired by that poll, the region's 'send them first' vanishes the moment it is said and the cashier stands at a focused Send with no reason on screen",
    find: "  return readTicket > note.raisedAt ? null : note;",
    replace: "  return null;",
  },
  {
    id: "staff-send-view/unsent-line-counts-a-stale-reading",
    file: "apps/qr/lib/staff-send-view.ts",
    suite: "lib/staff-send-view.test.ts",
    why: "Phase 2c · gate — once the page has read the drafts, its count is the live one. Preferring the refusal's own reading, a guest's third dish (landed and read) is said as two",
    find: "  return blocked || note.units === null ? detailUnits : note.units;",
    replace: "  return note.units === null ? detailUnits : note.units;",
  },
  {
    id: "pad/unsent-take-payment-live",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
  ...
  {
    id: "pad/unsent-outranks-waiting",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
  ...
  },
  {
    ...
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · gate — the pad's reason names HOW MANY dishes are still to send, from the server's count. Uncounted, it tells the cashier nothing they can check against the ticket",
    find: "  unsent: (c) => settleBlockedMsg(c.unsent, false),",
    replace: "  unsent: () => settleBlockedMsg(0, false),",
  },
  {
    id: "cashsettle/unsent-trigger-opens-the-sheet",
    file: "apps/qr/components/staff/CashSettleButton.tsx",
    suite: "components/staff/CashSettleButton.test.tsx",
    why: "Phase 2c · gate — aria-disabled does not stop a click; the handler's guard is the refusal. Without it the cash sheet opens over unsent dishes and the cashier counts change for a settle the server refuses",
    find: "          if (gateBlocked) {\n            // Opens no sheet: the page says why and takes the cashier to the Send.\n            onBlockedTap?.(null);\n            return;\n          }\n",
    replace: "",
  },
  {
    id: "cashsettle/unsent-trigger-looks-live",
    file: "apps/qr/components/staff/CashSettleButton.tsx",
    suite: "components/staff/CashSettleButton.test.tsx",
  ...
  {
    id: "cashsettle/unsent-trigger-always-dimmed",
    file: "apps/qr/components/staff/CashSettleButton.tsx",
    suite: "components/staff/CashSettleButton.test.tsx",
  ...
  {
    id: "cashsettle/unsent-refusal-never-jumps",
    file: "apps/qr/components/staff/CashSettleButton.tsx",
    suite: "components/staff/CashSettleButton.test.tsx",
    why: "Phase 2c · gate — the sheet's close after an `unsent` refusal hands the cashier to the Send. Without it they land back on the refused trigger with the fix somewhere above",
    find: "            if (units !== null && onBlockedTap) {\n              onBlockedTap(units);\n              return;\n            }\n",
    replace: "",
  },
  {
    id: "terminal-ui/unsent-tap-starts-the-reader",
    file: "apps/qr/components/staff/TerminalSettle.tsx",
    suite: "components/staff/TerminalSettle.test.tsx",
    why: "Phase 2c · gate — the reader's start refuses at the tap while dishes are unsent. Without the guard the reader is driven, a freeze taken and a PaymentIntent asked for, all to be refused",
    find: "    if (blocked) {\n      // Phase 2c · gate — refused at the tap: no reader, no freeze; the page names the fix.\n      setError(null);\n      onBlockedTap?.(null);\n      return;\n    }\n",
    replace: "",
  },
  {
    id: "terminal-ui/unsent-trigger-looks-live",
    file: "apps/qr/components/staff/TerminalSettle.tsx",
    suite: "components/staff/TerminalSettle.test.tsx",
  ...
  {
    id: "terminal-ui/unsent-said-in-english",
    file: "apps/qr/components/staff/TerminalSettle.tsx",
    suite: "components/staff/TerminalSettle.test.tsx",
  ...
  },
  {
    ...
    file: "apps/qr/components/staff/CloseSecureTabButton.tsx",
    suite: "components/staff/CloseSecureTabButton.test.tsx",
    why: "Phase 2c · gate — the running-bill close refuses at the tap while dishes are unsent. Without the guard the 'Charge $x' confirm opens, one tap from an off-session charge the server must refuse",
    find: "            if (blocked) {\n              // Opens no confirm: the page says why and takes staff to the lines.\n              setError(null);\n              onBlockedTap?.(null);\n              return;\n            }\n",
    replace: "",
  },
  {
    id: "secure-close/unsent-trigger-looks-live",
    file: "apps/qr/components/staff/CloseSecureTabButton.tsx",
    suite: "components/staff/CloseSecureTabButton.test.tsx",
  ...
  {
    id: "secure-close/unsent-said-in-english",
    file: "apps/qr/components/staff/CloseSecureTabButton.tsx",
    suite: "components/staff/CloseSecureTabButton.test.tsx",
  ...
  {
    id: "secure-close/unsent-refusal-never-jumps",
    file: "apps/qr/components/staff/CloseSecureTabButton.tsx",
    suite: "components/staff/CloseSecureTabButton.test.tsx",
    why: "Phase 2c · gate — a raced `unsent` refusal hands the jump up so the page takes staff to the lines (remove them if the guest has left). Dropped, they stay on a refused close",
    find: "        if (onBlockedTap) {\n          jumpOwnsFocus.current = true;\n          onBlockedTap(res.units);\n        }\n",
    replace: "",
  },
  {
    id: "secure-close/unsent-close-steals-focus-back",
    file: "apps/qr/components/staff/CloseSecureTabButton.tsx",
    suite: "components/staff/CloseSecureTabButton.test.tsx",
    why: "Phase 2c · gate — the confirm's close must not pull focus back to the trigger after the page's jump. Pulled back, the jump to the lines is undone a frame later",
    find: "    else if (!confirming && wasConfirming.current && !jumpOwnsFocus.current)",
    replace: "    else if (!confirming && wasConfirming.current)",
  },
  {
    id: "floor-detail/unsent-gate-not-passed",
  ...
  },
  {
    ...
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — a refused tap says why in the ONE region, visibly, at the settle rank. Never shown, the tap is silent and the cashier's focus lands on a Send with no reason",
    find: "            ) : gateLine ? (\n",
    replace: "            ) : false ? (\n",
  },
  {
    id: "floor-detail/unsent-note-dropped",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — the note under the triggers says why BEFORE anyone taps, and is what each dimmed trigger's description reads first. Dropped, the buttons are dimmed with no stated reason",
    find: "            {settleBlocked && (\n              <p id={SETTLE_UNSENT_NOTE_ID}",
    replace: "            {false && (\n              <p id={SETTLE_UNSENT_NOTE_ID}",
  },
  {
    id: "floor-detail/unsent-jump-leaves-focus",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — the refusal jumps to the fix. Without the focus move the cashier reads why with the finger still on a refused button",
    find: "      target?.focus({ preventScroll: true });\n",
    replace: "",
  },
  {
    id: "floor-detail/unsent-line-outlives-the-send",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — a send outcome is the newer fact. Kept, the gate's line outranks 'Sent' and says 'haven't gone to the kitchen' over dishes just sent",
    find: "      if (n) setSettleGate(null);\n",
    replace: "",
  },
  {
    id: "floor-detail/unsent-line-never-retires",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — the gate's line retires on a later read with nothing unsent (the dishes were removed). Never retired, the region keeps refusing a table the settle doors now accept",
    find: "    if (gate !== settleGate) setSettleGate(gate);\n",
    replace: "",
  },
  {
    id: "pad-ui/unsent-tap-leaves-focus",
    file: "apps/qr/components/staff/OrderPad.tsx",
    suite: "components/staff/OrderPad.test.tsx",
    why: "Phase 2c · gate — a refused Take payment takes the finger to the pad's Send. Without the focus move the cashier reads why on a dimmed button with the fix elsewhere on screen",
    find: "        send.controlRef.current?.focus({ preventScroll: true });\n",
    replace: "",
  },
  {
    id: "pad-ui/unsent-tap-stays-on-the-menu",
    file: "apps/qr/components/staff/OrderPad.tsx",
    suite: "components/staff/OrderPad.test.tsx",
  ...
  },
  {
    ...
    file: "apps/qr/components/Checkout.tsx",
    suite: "components/Checkout.test.tsx",
    why: "Phase 2c · gate — 'Pay at the counter' is the Bill's other door and keeps the Pay button's 'Everything sent' rule. Gated on the freeze alone, the family walks to the register over dishes nobody is cooking",
    find: "                disabled={payFrozen || sendBlocksPay}\n",
    replace: "                disabled={payFrozen}\n",
  },
  {
    id: "checkout/unsent-counter-tap-silent",
    file: "apps/qr/components/Checkout.tsx",
    suite: "components/Checkout.test.tsx",
    why: "Phase 2c · gate — a tap on the dimmed counter button repeats why. Silent, the diner taps a button that does nothing and reads nothing new",
    find: "                  sendBlocksPay && !payFrozen\n",
    replace: "                  false && !payFrozen\n",
  },
  // The gate's second round (the critic's findings): the raced line's lifetime, one sentence per
  // bill, the lines jump, the counter copy, and the rules round 1 left unmutated.
  {
    id: "settle/cash-unsent-read-before-the-freeze",
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/settle-unsent.test.ts",
    why: "Phase 2c · gate — the verdict is read UNDER the freeze. Read above the acquire, a guest's add (or a colleague's Send) lands between the verdict and the RPC: the cash order records a dish nobody sent, and the after() fire cooks it once the table has paid",
    find: '  const freeze = await acquireSettlementSuperseding(cart.id, attempt);\n  if (freeze !== "acquired") {\n',
    replace:
      '  const early = await kitchenDraftUnits(cart.id);\n  if (staffSettleBlockedByUnsent(session.mode, early)) return unsentRefusal(early);\n  const freeze = await acquireSettlementSuperseding(cart.id, attempt);\n  if (freeze !== "acquired") {\n',
  },
  {
    id: "settle/cash-unsent-read-after-the-totals",
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/settle-unsent.test.ts",
  ...
  },
  {
    ...
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/settle-unsent.test.ts",
    why: "Phase 2c · gate — the running-bill close reads the verdict UNDER the freeze. Above the acquire, a dish added between the verdict and the off-session charge rides the card on file for a guest who may have left",
    find: '  const freeze = await acquireSettlementSuperseding(cart.id, attempt);\n  if (freeze !== "acquired")\n    return {',
    replace:
      '  const early = await kitchenDraftUnits(cart.id);\n  if (staffSettleBlockedByUnsent(session.mode, early)) return unsentRefusal(early);\n  const freeze = await acquireSettlementSuperseding(cart.id, attempt);\n  if (freeze !== "acquired")\n    return {',
  },
  {
    id: "settle/tab-close-unsent-read-after-the-totals",
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/settle-unsent.test.ts",
  ...
  },
  {
    ...
    file: "apps/qr/lib/terminal.ts",
    suite: "lib/terminal.test.ts",
    why: "Phase 2c · gate — the reader's verdict is read UNDER the freeze. Above the acquire, a dish added between the verdict and the PaymentIntent is charged on the reader and cooked after the table has paid",
    find: "  const freeze = await acquireSettlementSuperseding(cart.id, attemptId);\n",
    replace:
      "  const early = await kitchenDraftUnits(cart.id);\n  if (staffSettleBlockedByUnsent(session.mode, early)) return unsentRefusal(early);\n  const freeze = await acquireSettlementSuperseding(cart.id, attemptId);\n",
  },
  {
    id: "terminal/card-unsent-read-after-the-totals",
    file: "apps/qr/lib/terminal.ts",
    suite: "lib/terminal.test.ts",
  ...
  },
  {
    ...
    file: "apps/qr/components/staff/TerminalSettle.tsx",
    suite: "components/staff/TerminalSettle.test.tsx",
    why: "Phase 2c · gate — the gate dims the reader only while it holds. Always spread, every reader start reads as refused to a screen reader",
    find: '        {...(blocked ? { "aria-disabled": true } : {})}\n',
    replace: '        {...{ "aria-disabled": true }}\n',
  },
  {
    id: "secure-close/unsent-trigger-always-dimmed",
    file: "apps/qr/components/staff/CloseSecureTabButton.tsx",
    suite: "components/staff/CloseSecureTabButton.test.tsx",
    why: "Phase 2c · gate — the gate dims the card-on-file close only while it holds. Always spread, every running-bill close reads as refused",
    find: '          {...(blocked ? { "aria-disabled": true } : {})}\n',
    replace: '          {...{ "aria-disabled": true }}\n',
  },
  {
    id: "floor-detail/unsent-reader-gate-not-passed",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — the page hands the gate to EVERY trigger. Not passed to the reader, it starts a card-present charge over unsent dishes (refused by the server one round trip later, focus left on it)",
    find: '                blocked={settleBlocked}\n                blockedNoteId={SETTLE_UNSENT_NOTE_ID}\n                onBlockedTap={(units) => onSettleBlocked("reader", units)}',
    replace:
      '                blocked={false}\n                blockedNoteId={SETTLE_UNSENT_NOTE_ID}\n                onBlockedTap={(units) => onSettleBlocked("reader", units)}',
  },
  {
    id: "floor-detail/unsent-close-gate-not-passed",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — not passed to the running-bill close, its 'Charge $x' confirm opens over unsent dishes, one tap from an off-session charge the server must refuse",
    find: '                blocked={settleBlocked}\n                blockedNoteId={SETTLE_UNSENT_NOTE_ID}\n                onBlockedTap={(units) => onSettleBlocked("tab", units)}',
    replace:
      '                blocked={false}\n                blockedNoteId={SETTLE_UNSENT_NOTE_ID}\n                onBlockedTap={(units) => onSettleBlocked("tab", units)}',
  },
  {
    id: "pad/unsent-outranks-paying",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · gate — a guest paying outranks the gate: the Send is refused behind the same payment, so 'send them first' names a fix that cannot be done",
    find: '  const block: PadSettleBlock | null = i.paying\n    ? "paying"',
    replace:
      '  const block: PadSettleBlock | null = staffSettleBlockedByUnsent(i.mode, i.unsentUnits)\n    ? "unsent"\n    : i.paying\n    ? "paying"',
  },
  {
    id: "pad/unsent-outranks-note",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · gate — an unsaved note outranks the gate: the Send is held on that very note (drain before fire), so the fix is saving it, not 'send them first'",
    find: '    : i.unsavedNote\n      ? "note"',
    replace:
      '    : staffSettleBlockedByUnsent(i.mode, i.unsentUnits)\n      ? "unsent"\n      : i.unsavedNote\n      ? "note"',
  },
  {
    id: "counter-pay/unsent-host-read-fails-closed",
    file: "apps/qr/lib/counter-pay.ts",
    suite: "lib/counter-pay.test.ts",
    why: "Phase 2c · gate — the host read fails OPEN: the ask moves no money (the register's settle doors and create-intent each refuse on their own). Read as a present host on a blip, a family is held at the table by a read nobody could make",
    find: "        return error ? null : (data?.host_seat ?? null);\n",
    replace: '        return error ? "unreadable" : (data?.host_seat ?? null);\n',
  },
  {
    id: "terminal-ui/unsent-raced-line-never-dropped",
    file: "apps/qr/components/staff/TerminalSettle.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — a raced refusal's line is DROPPED once the page catches up. Kept (only hidden while blocked), it comes back under a live 'Card on the reader' after the dishes were sent, saying they have not been (critic finding)",
    find: '  if (error?.kind === "unsent" && (blocked || gateLive === false)) setError(null);\n',
    replace: "",
  },
  {
    id: "terminal-ui/unsent-raced-line-outlives-the-page",
    file: "apps/qr/components/staff/TerminalSettle.tsx",
    suite: "components/staff/TerminalSettle.test.tsx",
    why: "Phase 2c · gate — once the page reads the table blocked, its note owns the sentence; the raced line is dropped then, so a later clear read cannot bring it back",
    find: "(blocked || gateLive === false)",
    replace: "gateLive === false",
  },
  {
    id: "terminal-ui/unsent-raced-line-outlives-the-gate",
    file: "apps/qr/components/staff/TerminalSettle.tsx",
    suite: "components/staff/TerminalSettle.test.tsx",
    why: "Phase 2c · gate — the raced line never outlives the page's own gate line: dishes removed before the page ever read them leave the reader saying they are still there",
    find: "(blocked || gateLive === false)",
    replace: "blocked",
  },
  {
    id: "secure-close/unsent-raced-line-never-dropped",
    file: "apps/qr/components/staff/CloseSecureTabButton.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — the close's raced line is DROPPED once the page catches up. Kept, it comes back after the dishes were removed, offering to remove them (critic finding)",
    find: '  if (error?.kind === "unsent" && (blocked || gateLive === false)) setError(null);\n',
    replace: "",
  },
  {
    id: "secure-close/unsent-raced-line-outlives-the-page",
    file: "apps/qr/components/staff/CloseSecureTabButton.tsx",
    suite: "components/staff/CloseSecureTabButton.test.tsx",
    why: "Phase 2c · gate — once the page reads the table blocked its note owns the sentence; the close's raced line is dropped then, never merely hidden",
    find: "(blocked || gateLive === false)",
    replace: "gateLive === false",
  },
  {
    id: "secure-close/unsent-raced-line-outlives-the-gate",
    file: "apps/qr/components/staff/CloseSecureTabButton.tsx",
    suite: "components/staff/CloseSecureTabButton.test.tsx",
    why: "Phase 2c · gate — the close's raced line never outlives the page's own gate line (dishes removed before the page read them)",
    find: "(blocked || gateLive === false)",
    replace: "blocked",
  },
  {
    id: "terminal-ui/unsent-running-ignored",
    file: "apps/qr/components/staff/TerminalSettle.tsx",
    suite: "components/staff/TerminalSettle.test.tsx",
    why: "Phase 2c · gate — on a card-on-file running bill the reader's raced line says the running bill's sentence, the note's. Ignored, it says 'send them first' under a note offering removal: two sentences for one fact",
    find: "            k={settleBlockedMsg(error.units, running).k}\n",
    replace: "            k={settleBlockedMsg(error.units, false).k}\n",
  },
  {
    id: "cashsettle/unsent-running-ignored",
    file: "apps/qr/components/staff/CashSettleButton.tsx",
    suite: "components/staff/CashSettleButton.test.tsx",
    why: "Phase 2c · gate — the cash sheet's raced refusal says the bill's sentence (the page's note's, and the region's after the close). Ignored, a running bill hears two different sentences for one fact",
    find: "                      k={settleBlockedMsg(alertMsg.units, running).k}\n",
    replace: "                      k={settleBlockedMsg(alertMsg.units, false).k}\n",
  },
  {
    id: "floor-detail/unsent-note-variant-ignored",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — the note offers 'remove them if the guest has left' on a card-on-file running bill, where the close may run for a guest who has gone",
    find: "                    k={settleBlockedMsg(detail.send.sendable, runningClose).k}\n",
    replace: "                    k={settleBlockedMsg(detail.send.sendable, false).k}\n",
  },
  {
    id: "floor-detail/unsent-region-variant-by-trigger",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — the region's sentence is the BILL's (one binding), not the tapped door's. By door, a cash tap on a running bill says 'send them first, then take payment' under a note offering removal (critic finding)",
    find: "        settleGateUnits(settleGate, settleBlocked, detail.send.sendable),\n        runningClose,\n",
    replace:
      '        settleGateUnits(settleGate, settleBlocked, detail.send.sendable),\n        settleGate.trigger === "tab",\n',
  },
  {
    id: "floor-detail/unsent-reader-variant-not-passed",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — the page hands the bill's variant to the reader; not passed, its raced line says the table's sentence on a running bill",
    find: 'onSettleBlocked("reader", units)}\n                running={runningClose}\n',
    replace: 'onSettleBlocked("reader", units)}\n                running={false}\n',
  },
  {
    id: "floor-detail/unsent-cash-variant-not-passed",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — the page hands the bill's variant to cash; not passed, the sheet's raced refusal says the table's sentence on a running bill",
    find: 'onSettleBlocked("cash", units)}\n              running={runningClose}\n',
    replace: 'onSettleBlocked("cash", units)}\n              running={false}\n',
  },
  {
    id: "floor-detail/unsent-lines-jump-centred",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · gate — the jump to the order's lines puts the heading at the TOP, the lines to remove below it. Centred, half a phone screen goes to the card above and the reason at the order's foot is pushed further off (critic finding)",
    find: '        block: target === orderHeadingRef.current ? "start" : "center",\n',
    replace: '        block: "center",\n',
  },
  {
    id: "counter-pay-state/unsent-guest-told-to-send",
    file: "apps/qr/lib/counter-pay-state.ts",
    suite: "lib/counter-pay-state.test.ts",
    why: "Phase 2c · gate — only the host can send. The host's sentence for everyone tells a guest to send dishes they cannot (critic finding)",
    find: "  return sender === null\n",
    replace: "  return true\n",
  },
  {
    id: "checkout/unsent-counter-guest-told-to-send",
    file: "apps/qr/components/Checkout.tsx",
    suite: "components/Checkout.test.tsx",
  ...
  {
    id: "pad2/dish-hold-ignores-lost",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · review fixes · pad2 (P1) — a LOST add holds its dish. Ignored, the tile stays live, the re-tap mints a new add key, and if the first landed the table is billed and cooked a second plate",
    find: '(p.state === "lost" || p.state === "unconfirmed")',
    replace: 'p.state === "unconfirmed"',
  },
  {
    id: "pad2/dish-hold-by-any-dish",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · review fixes · pad2 (P1) — the hold is THIS dish's. Any dish's unknown add holding every tile stalls the whole order on one dish",
    find: "p.itemId === itemId && (",
    replace: "(",
  },
  {
    id: "pad2/dish-hold-blocks-its-own-retry",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · review fixes · pad2 (P1) — the held attempt's OWN key passes: the options sheet's same choice again is the one safe retry (the ledger dedupes it). Refused by its own hold, the only way out is a reload",
    find: " && p.key !== key,",
    replace: ",",
  },
  {
    id: "pad2/tile-held-dish-live",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · review fixes · pad2 (P1) — a held dish's tile reads refused (aria-disabled). Live, it invites the tap that is a second plate",
    find: '  if (i.dishHeld) return "held";\n',
    replace: "",
  },
  {
    id: "pad2/tile-live-while-settling",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · review fixes · pad2 (P4) — Take payment on its way out refuses every tile. Live, a dish tapped while the name saves or payment opens is added after the drain and said to a screen that is leaving",
    find: '  if (i.settling) return "settling";\n',
    replace: "",
  },
  {
    id: "pad2/view-lost-said-adding",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: 'Phase 2c · review fixes · pad2 (P8) — a lost add\'s answer CAME: the view button names the fix. Dropped, it falls to nothing on its own, or reads "Adding…" beside a flying one',
    find: '  if (p.lost > 0) return "pad.bar.check";\n',
    replace: "",
  },
  {
    id: "pad2/view-unconfirmed-said-adding",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: 'Phase 2c · review fixes · pad2 (P8) — 15s unanswered is "Checking…", the ghost\'s own word, never "Adding…"',
    find: '  if (p.unconfirmed > 0) return "pad.ghost.checking";\n',
    replace: "",
  },
  {
    id: "pad2/name-empty-reads-saved",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: 'Phase 2c · review fixes · pad2 (P11) — an empty field over no saved name has nothing to save. Read as saved, Save wears "Saved ✓" over nothing',
    find: '  return v === "" ? "empty" : "saved";',
    replace: '  return "saved";',
  },
  {
    id: "pad2/name-clear-refused",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · review fixes · pad2 (P11) — an emptied field over a saved name CLEARS it. Read as empty, the old name can never be taken off the order",
    find: '  if (v !== saved) return "save";',
    replace: '  if (v !== saved && v !== "") return "save";',
  },
  {
    id: "pad2/retry-refusal-read-as-definite",
    file: "apps/qr/lib/pad-errors.ts",
    suite: "lib/pad-errors.test.ts",
    why: 'Phase 2c · review fixes · pad2 (P2) — a RETRY refused before the ledger says nothing about the first attempt. Read as definite, the ghost goes, the key is dropped and "didn\'t go on" invites a new tap under a new key — a second plate',
    find: '  if (v.kind !== "refused" || v.err.kind === "signin") return v;',
    replace: '  if (v.kind !== "refused" || v.err.kind !== "signin") return v;',
  },
  {
    id: "pad2/retry-outage-said-as-failed",
    file: "apps/qr/lib/pad-errors.ts",
    suite: "lib/pad-errors.test.ts",
    why: "Phase 2c · review fixes · pad2 (P2) — the retry says WHY it could not run: an outage by name. Collapsed into the generic sentence, the fix (the connection) goes unsaid",
    find: 'code === "outage"\n        ? RETRY_ERR.outage\n        : ',
    replace: "",
  },
  {
    id: "pad2/retry-reads-first-attempt",
    file: "apps/qr/components/staff/usePadWrites.ts",
    suite: "components/staff/OrderPad.test.tsx",
    why: 'Phase 2c · review fixes · pad2 (P2) — the chain reads a retry\'s answer through padRetryVerdict. Read raw, a pre-ledger refusal of "Try again" drops the ghost and its key: the next tap is a second plate',
    find: "const v = retried.current.has(key) ? padRetryVerdict(answer) : answer;",
    replace: "const v = answer;",
  },
  {
    id: "pad2/ghost-retry-said-to-the-sheet",
    file: "apps/qr/components/staff/usePadWrites.ts",
    suite: "components/staff/OrderPad.test.tsx",
    why: "Phase 2c · review fixes · pad2 (P2) — Try again on the ghost of a SHEET add is the pad's to say. Kept quiet, its refusal is said to a sheet that is closed — to nobody",
    find: "      else quiet.current.delete(key);\n",
    replace: "",
  },
  {
    id: "pad2/retry-ok-silent",
    file: "apps/qr/components/staff/usePadWrites.ts",
    suite: "components/staff/OrderPad.test.tsx",
    why: "Phase 2c · review fixes · pad2 (P6) — an add whose doubt was said is said to LAND. Silent, the region's last word stays \"we couldn't confirm\" over a dish that is on",
    find: "if (announced.current.has(key) && !quiet.current.has(key))",
    replace: "if (false)",
  },
  {
    id: "pad2/hang-said-to-nobody",
    file: "apps/qr/components/staff/usePadWrites.ts",
    suite: "components/staff/OrderPad.test.tsx",
    why: 'Phase 2c · review fixes · pad2 (P6) — 15s unanswered is SAID once. Silent, the ghost\'s "Checking…" is aria-hidden and a screen-reader user last heard "Added"',
    find: "                if (!originWaiting) {",
    replace: "                if (false) {",
  },
  {
    id: "pad2/tile-tap-ignores-the-dish-hold",
    file: "apps/qr/components/staff/OrderPad.tsx",
    suite: "components/staff/OrderPad.test.tsx",
    why: "Phase 2c · review fixes · pad2 (P1) — the tap decides from the chain AT THE TAP. Blind to the dish's hold, a re-tap on a lost dish mints a new key — a second plate",
    find: "        dishHeld: dishHold !== null,\n",
    replace: "        dishHeld: false,\n",
  },
  {
    id: "pad2/tap-ignores-the-settle",
    file: "apps/qr/components/staff/OrderPad.tsx",
    suite: "components/staff/OrderPad.test.tsx",
    why: "Phase 2c · review fixes · pad2 (P4) — the tap reads Take payment's phase from the ref. Blind to it, a dish is added while the page leaves",
    find: '        settling: phaseRef.current !== "idle",\n',
    replace: "        settling: false,\n",
  },
  {
    id: "pad2/sheet-new-key-over-a-held-dish",
    file: "apps/qr/components/staff/OrderPad.tsx",
    suite: "components/staff/OrderPad.test.tsx",
    why: "Phase 2c · review fixes · pad2 (P1) — a new sheet choice for a dish whose add is unknown is refused. Minted, it is a second curry if the first landed",
    find: "    const dishHold = writes.holdFor(item.id, key);",
    replace: "    const dishHold = null as ReturnType<typeof writes.holdFor>;",
  },
  {
    id: "pad2/name-empty-tap-silent",
    file: "apps/qr/components/staff/OrderPad.tsx",
    suite: "components/staff/OrderPad.test.tsx",
    why: "Phase 2c · review fixes · pad2 (P11) — a refused Save says why once (§17). Silent, a live-looking button does nothing",
    find: '                    if (nameSave === "empty") say({ k: "pad.name.empty" });\n',
    replace: "",
  },
  {
    id: "pad2/send-fires-past-a-late-note",
    file: "apps/qr/components/staff/useStaffSend.ts",
    suite: "components/staff/OrderPad.test.tsx",
    why: "Phase 2c · review fixes · pad2 (P3) — the hold is read AGAIN after the drain. Read only at the tap, a kitchen note typed while the Send waited is fired past, its save refused (draft-only) — the allergy lost",
    find: "          const late = getHold();",
    replace: "          const late = null as StaffSendHold;",
  },
  {
    id: "pad2/detail-read-piles-on-a-hung-one",
    file: "apps/qr/components/staff/usePadDetailLive.ts",
    suite: "components/staff/OrderPad.test.tsx",
    why: "Phase 2c · review fixes · pad2 (P5) — no read starts over a hung raw read. Without the guard each poll stacks another abandoned call in Next's one-at-a-time queue",
    find: "    if (rawPending.current) {\n      owed.current = true;\n      return;\n    }\n",
    replace: "",
  },
  {
    id: "pad2/owed-read-never-kicked",
    file: "apps/qr/components/staff/usePadDetailLive.ts",
    suite: "components/staff/OrderPad.test.tsx",
    why: "Phase 2c · review fixes · pad2 (P5) — the asks a hung read refused are owed ONE read at its answer. Never kicked, a landed ghost waits for the next 5s poll",
    find: "              kick.current();\n",
    replace: "",
  },
  // ── Phase 2c · review fixes · reg2 ──
  // The blind review's REGISTER / GATE findings (R1–R6 and the open questions), each killed by its
  // own suite, value-falsified. Ids share `p2c-reg2/` so `--only=p2c-reg2` runs the block.
  {
    id: "p2c-reg2/quote-refusal-outlives-a-later-read",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · review (R1) — a refusal's figure is settled by a read that began AFTER it. Without the ticket rule, a guest who adds and removes a dish before the re-read brings the live total back to the basis and the quote sticks on a total that no longer exists, in silence",
    find: "  if (q && q.basis !== q.cents && q.raisedAt !== undefined && readTicket > q.raisedAt)\n    return { cents: q.cents, basis: q.cents };\n",
    replace: "",
  },
  {
    id: "p2c-reg2/quote-settled-by-a-read-in-the-air",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · review (R1) — a read already in the air when the refusal came back may predate the move; letting it settle the quote drops the server's figure for a stale one and says a false 'changed' sentence",
    find: "readTicket > q.raisedAt)",
    replace: "readTicket >= q.raisedAt)",
  },
  {
    id: "p2c-reg2/cash-refusal-raised-at-the-committed-read",
    file: "apps/qr/components/staff/CashSettleButton.tsx",
    suite: "components/staff/CashSettleButton.test.tsx",
    why: "Phase 2c · review (R1) — the cash sheet marks a refusal with the last read STARTED; marked with the committed ticket, a read in the air at the refusal settles it and a false drift back to the old figure is said",
    find: "raisedAt: readsStarted?.() ?? readTicket });",
    replace: "raisedAt: readTicket });",
  },
  {
    id: "p2c-reg2/cash-quote-ignores-the-read-clock",
    file: "apps/qr/components/staff/CashSettleButton.tsx",
    suite: "components/staff/CashSettleButton.test.tsx",
    why: "Phase 2c · review (R1) — the cash sheet reconciles its quote against the page's read ticket; without it the add-then-remove quote sticks on the server's figure",
    find: "reconcileQuote(quote, totalCents, readTicket);",
    replace: "reconcileQuote(quote, totalCents);",
  },
  {
    id: "p2c-reg2/tab-close-refusal-raised-at-the-committed-read",
    file: "apps/qr/components/staff/CloseSecureTabButton.tsx",
    suite: "components/staff/CloseSecureTabButton.test.tsx",
    why: "Phase 2c · review (R1) — the running-bill close marks a refusal with the last read STARTED; marked with the committed ticket, a read in the air at the refusal puts the pre-refusal figure back on the trigger",
    find: "raisedAt: readsStarted?.() ?? readTicket });",
    replace: "raisedAt: readTicket });",
  },
  {
    id: "p2c-reg2/tab-close-quote-ignores-the-read-clock",
    file: "apps/qr/components/staff/CloseSecureTabButton.tsx",
    suite: "components/staff/CloseSecureTabButton.test.tsx",
    why: "Phase 2c · review (R1) — the running-bill close reconciles its quote against the page's read ticket; without it the trigger keeps the server's stale figure",
    find: "reconcileQuote(quote, totalCents, readTicket);",
    replace: "reconcileQuote(quote, totalCents);",
  },
  {
    id: "p2c-reg2/floor-cash-reads-started-dropped",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · review (R1) — the page hands the cash sheet its read-started clock; without it a refusal is marked with the committed ticket and a read in the air settles it",
    find: "              running={runningClose}\n              readTicket={readTicket}\n              readsStarted={readsStarted}\n",
    replace: "              running={runningClose}\n              readTicket={readTicket}\n",
  },
  {
    id: "p2c-reg2/floor-cash-read-ticket-dropped",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · review (R1) — the page hands the cash sheet its read ticket; without it no read ever settles a refusal's figure",
    find: "              running={runningClose}\n              readTicket={readTicket}\n              readsStarted={readsStarted}\n",
    replace: "              running={runningClose}\n              readsStarted={readsStarted}\n",
  },
  {
    id: "p2c-reg2/floor-close-reads-started-dropped",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · review (R1) — the page hands the running-bill close its read-started clock; without it a read in the air settles the refusal",
    find: 'onSettleBlocked("tab", units)}\n                readTicket={readTicket}\n                readsStarted={readsStarted}\n',
    replace: 'onSettleBlocked("tab", units)}\n                readTicket={readTicket}\n',
  },
  {
    id: "p2c-reg2/floor-close-read-ticket-dropped",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · review (R1) — the page hands the running-bill close its read ticket; without it the trigger keeps the server's stale figure forever",
    find: 'onSettleBlocked("tab", units)}\n                readTicket={readTicket}\n                readsStarted={readsStarted}\n',
    replace: 'onSettleBlocked("tab", units)}\n                readsStarted={readsStarted}\n',
  },
  {
    id: "p2c-reg2/unknown-never-cleared-by-a-read",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · review (R2) — an open cart read after the lost settle could last land proves it never did; never cleared, a close hours later is said as 'the payment most likely went through'",
    find: "  return read.cartOpen && read.startedAtMs > since + SETTLE_MAY_LAND_MS ? null : since;\n",
    replace: "  return since;\n",
  },
  {
    id: "p2c-reg2/unknown-cleared-at-the-boundary",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · review (R2) — a read that starts at the last instant the settle could land proves nothing",
    find: "read.startedAtMs > since",
    replace: "read.startedAtMs >= since",
  },
  {
    id: "p2c-reg2/unknown-cleared-by-any-open-read",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · review (R2) — the re-read right after the rejection may predate the commit; clearing on it bounces a late-landing settle's close to the floor",
    find: "since + SETTLE_MAY_LAND_MS ? null",
    replace: "since ? null",
  },
  {
    id: "p2c-reg2/unknown-cleared-by-a-paid-read",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · review (R2) — a paid or gone cart is what a landed settle looks like; clearing on it drops the 'most likely went through' exactly when it is true",
    find: "read.cartOpen && read.startedAtMs",
    replace: "read.startedAtMs",
  },
  {
    id: "p2c-reg2/unknown-bound-not-the-freeze",
    file: "apps/qr/lib/register-math.ts",
    suite: "lib/register-math.test.ts",
    why: "Phase 2c · review (R2) — the bound is the settle freeze's own lifetime (one binding); a shorter invented number clears the mark while the settle may still land",
    find: "export const SETTLE_MAY_LAND_MS = SETTLE_TTL_MS;",
    replace: "export const SETTLE_MAY_LAND_MS = 30_000;",
  },
  {
    id: "p2c-reg2/floor-unknown-survives-the-reader",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · review (R2) — a reader collect that STARTS took the freeze on an open cart, so the lost cash settle can no longer land; kept, the reader's own close is said as the cash having most likely gone through",
    find: "      if (c) settleUnknown.current = null;\n",
    replace: "",
  },
  {
    id: "p2c-reg2/floor-unknown-never-read-out",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · review (R2) — the page runs every committed detail read through settleUnknownAfterRead; skipped, the mark never ends",
    find: "            settleUnknown.current = settleUnknownAfterRead(settleUnknown.current, {\n              startedAtMs,\n              cartOpen: res.detail.cartId != null && !res.detail.settled,\n            });\n",
    replace: "",
  },
  {
    id: "p2c-reg2/floor-unknown-read-timed-at-its-answer",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · review (R2) — a read is timed by when it STARTED; timed by its answer, a read begun inside the window clears the mark though the settle may have landed after its snapshot",
    find: "              startedAtMs,\n              cartOpen:",
    replace: "              startedAtMs: Date.now(),\n              cartOpen:",
  },
  {
    id: "p2c-reg2/floor-unknown-dated-at-the-epoch",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · review (R2) — the mark is dated when the page learned the outcome was unknown; dated at 0, the very next open read clears it and a late landing bounces to the floor",
    find: "settleUnknown.current = unknown ? Date.now() : null;",
    replace: "settleUnknown.current = unknown ? 0 : null;",
  },
  {
    id: "p2c-reg2/floor-unknown-cleared-by-a-paid-read",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · review (R2) — the page reads a paid (settled / cart-less) detail as NOT open; read as open, the landed settle's paid reads clear the mark and its close bounces to the floor",
    find: "cartOpen: res.detail.cartId != null && !res.detail.settled,",
    replace: "cartOpen: true,",
  },
  {
    id: "p2c-reg2/floor-reader-status-under-a-stale-refusal",
    file: "apps/qr/components/staff/FloorDetailLive.tsx",
    suite: "components/staff/FloorDetailLive.test.tsx",
    why: "Phase 2c · review (R3) — each reader status change clears a standing writeError; kept, a line-edit refusal from before the collect outranks the reader's money sentences in the one region forever",
    find: "    setReaderStatus(s);\n    setWriteError(null);\n",
    replace: "    setReaderStatus(s);\n",
  },
  {
    id: "p2c-reg2/cas-suite-cash-gate-deleted",
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/settle-cash-cas.test.ts",
    why: "Phase 2c · review (R5) — the CAS suite answers the gate's read; the cash gate deleted must turn IT red too (it used to run every case through the fail-open branch)",
    find: "    if (staffSettleBlockedByUnsent(session.mode, unsentUnits)) return unsentRefusal(unsentUnits);\n",
    replace: "",
  },
  {
    id: "p2c-reg2/cas-suite-cash-gate-after-the-cas",
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/settle-cash-cas.test.ts",
  ...
  {
    id: "p2c-reg2/cas-suite-close-gate-after-the-cas",
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/secure-close-cas.test.ts",
  ...
  },
  {
    ...
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/settle-unsent.test.ts",
    why: "Phase 2c · review (R6) — the counter's exemption is the session's MODE; read as dine-in, a counter order holding dine-in drafts can never take cash",
    find: "    if (staffSettleBlockedByUnsent(session.mode, unsentUnits)) return unsentRefusal",
    replace: '    if (staffSettleBlockedByUnsent("dinein", unsentUnits)) return unsentRefusal',
  },
  {
    id: "p2c-reg2/tab-close-gate-ignores-the-mode",
    file: "apps/qr/lib/staff-cart.ts",
    suite: "lib/settle-unsent.test.ts",
    why: "Phase 2c · review (R6) — the running-bill close reads the same MODE; read as dine-in, the counter is gated where it cooks at payment",
    find: "  if (staffSettleBlockedByUnsent(session.mode, unsentUnits)) {\n    await releaseSettlementFor(cart.id, attempt);",
    replace:
