...
const MUTANTS = [
  ...
  },
  {
    ...
    why: "T21(b) partial \u2014 the ticket is the only thing making the cart view monotonic. Several concurrent getCartView calls are in flight at once with no cancellation between them, so accepting a ticket that is merely NOT AHEAD lets the slower of two reads win: a read that observed `locked: true` lands after one that observed the lock released, puts the freeze back, and the /menu surface stays dead for another full TTL with no event able to clear it",
    find: "  return seq === s.issued;",
    replace: "  return seq <= s.issued;",
  },
  {
    id: "seq/a-mutations-view-stops-invalidating-reads",
    file: "apps/qr/lib/view-seq.ts",
    suite: "lib/view-seq.test.ts",
    why: "T21(b) partial \u2014 a mutation's returned view is rendered server-side inside the statement that committed the write, so it is strictly fresher than any client read that merely started earlier. Applying it WITHOUT bumping the counter leaves those older reads valid, and the next one to land undoes the add the diner just watched appear \u2014 the optimistic-doctrine snap-back this provider exists to avoid",
