...
const MUTANTS = [
  ...
  {
    id: "undo-hold/the-cap-never-warns",
    file: "apps/qr/lib/undo-hold.ts",
    suite: "lib/undo-hold.test.ts",
    why: "Blind review (2026-09-24) — a keyboard user parked on Undo is told, five seconds before the minute's cap, that the pick is about to go through. Without the warning the bag leaves the tracker and the wall under them with no word first",
    find: '  if (held >= capMs - warnMs) return "warn";\n',
    replace: "",
  },
  {
    id: "undo-hold/the-cap-never-releases",
    file: "apps/qr/lib/undo-hold.ts",
    suite: "lib/undo-hold.test.ts",
    why: "Blind review (2026-09-24) — at the cap the hold is LET GO, so the pill's drain visibly runs again. Never released, the pill keeps data-held (a paused drain) over a window that is really closing — it claims a hold the lane no longer honours",
    find: '  if (held >= capMs) return "release";\n',
    replace: "",
  },
  {
    id: "undo-hold/a-spent-hold-re-holds",
    file: "apps/qr/lib/undo-hold.ts",
    suite: "lib/undo-hold.test.ts",
    why: "Blind review (2026-09-24) — a hold released at its cap is spent. A fresh keyboard focus re-holding it pauses the drain again over a window that keeps running",
    find: "    if (h.heldMs >= capMs) return h;\n",
    replace: "",
  },
  {
    id: "undo-hold/the-release-forgets-the-hold",
    file: "apps/qr/lib/undo-hold.ts",
    suite: "lib/undo-hold.test.ts",
    why: "Blind review (2026-09-24) — the cap's release keeps the full minute as time held. Dropping the open hold's minute re-opens a six-second window that already ran, so the pick lands later than the warning said",
    find: "  return { sources: new Set(), since: null, heldMs: heldFor(h, now, capMs) };",
    replace: "  return { sources: new Set(), since: null, heldMs: h.heldMs };",
  },
  {
    id: "undo-hold/the-prune-keeps-dead-entries",
    file: "apps/qr/lib/undo-hold.ts",
    suite: "lib/undo-hold.test.ts",
    why: "Blind review (2026-09-24) — the poll's prune DELETES every per-window entry of a bag that left the lane. Kept, a lane that runs all shift carries one dead hold and one dead undo stamp per bag it ever picked",
    find: "  for (const id of gone) m.delete(id);\n",
    replace: "",
  },
  {
  ...
  // ── Phase 2c · pad ──
  // The order pad's pure rules (lib/order-pad · pad-pending · pad-errors), the quick-add gate
  // (needsChoice), and the fields getTableDetail hands the pad's ticket. Each is killed by its own
  // suite, value-falsified; the components are held by their jsdom suites.
  {
    id: "pad/needs-choice-counts-optional-groups",
    file: "apps/qr/lib/menu/modifiers.ts",
    suite: "lib/menu/modifiers.test.ts",
    why: "Phase 2c · pad — only a REQUIRED group is a choice. Counting any group sends Mohinga, Mee-Shay and Shan Noodles (optional spice/add-ons, most of the adds in a service) through a sheet: two taps where one would do",
    find: "  return groups.some((g) => g.minSelect >= 1);",
    replace: "  return groups.length > 0;",
  },
  {
    id: "pad/needs-choice-off-by-one",
    file: "apps/qr/lib/menu/modifiers.ts",
    suite: "lib/menu/modifiers.test.ts",
    why: "Phase 2c · pad — a min-1 group IS a choice. Off by one, a curry is quick-added with no style and every add is refused by the server's cardinality check",
    find: "  return groups.some((g) => g.minSelect >= 1);",
    replace: "  return groups.some((g) => g.minSelect > 1);",
  },
  {
    id: "pad/sold-out-below-the-choice",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — sold out wins over a required choice. Checked second, a sold-out curry opens a sheet whose every Add the server refuses",
    find: '  if (item.soldOut) return "soldOut";\n  if (needsChoice(item.groups)) return "choose";',
    replace:
      '  if (needsChoice(item.groups)) return "choose";\n  if (item.soldOut) return "soldOut";',
  },
  {
    id: "pad/fallback-english-marked-burmese",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — with no Burmese on a Burmese console the English leads marked `en`. Marked with the console's tongue, it is set in Padauk and announced as Burmese",
    find: '  if (my === null) return { lead: { text: nameEn, lang: "en" }, echo: null };',
    replace: "  if (my === null) return { lead: { text: nameEn, lang }, echo: null };",
  },
  {
    id: "pad/raw-name-my-trusted",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — a Burmese name is a catalog FACT or nothing (`catalogNameMy`). Trusted raw, a romanisation stored as name_my leads the tile as 'Burmese'",
    find: "  const my = catalogNameMy(nameMyRaw, nameEn);",
    replace: "  const my = nameMyRaw;",
  },
  {
    id: "pad/rail-sorted-by-name",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — the rail and the sections follow the menu's sort_order. By name, Curries leads a menu that opens with breakfast",
    find: "    .sort((a, b) => a[1].sort - b[1].sort || a[1].at - b[1].at)",
    replace: "    .sort((a, b) => a[1].title.localeCompare(b[1].title))",
  },
  {
    id: "pad/search-keeps-the-chip-lit",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — a search un-presses every chip. A chip lit over a search claims a filter that is not applied",
    find: '      activeCat: null,\n      sections: [{ key: "search"',
    replace: '      activeCat: opts.cat,\n      sections: [{ key: "search"',
  },
  {
    id: "pad/search-applies-the-chip",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — a search spans the whole menu. Filtered by the pressed chip, 'curry' under Sides finds nothing and the search dead-ends (the old browser's bug)",
    find: "browseRows(items, opts.q, false)",
    replace:
      "browseRows(\n          items.filter((i) => opts.cat === null || i.categorySlug === opts.cat),\n          opts.q,\n          false,\n        )",
  },
  {
    id: "pad/badge-counts-voided-lines",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — the tile's ×N counts what is on the order. With voided lines in it, a removed dish still badges and a server re-adds nothing, thinking it is there",
    find: '    if (l.state === "voided" || l.menuItemId === null) continue;',
    replace: "    if (l.menuItemId === null) continue;",
  },
  {
    id: "pad/togo-draft-reads-unsent",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — a to-go draft at a dine-in table is its own group. Under 'Not sent yet' it tells staff to send a dish the Send will not fire",
    find: '  return mode === "dinein" && l.fulfillment === "togo" ? "togo" : "unsent";',
    replace: '  return "unsent";',
  },
  {
    id: "pad/a-lone-group-heading",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — headings only with two groups or more; one heading over every line says nothing and costs a row",
    find: "  return { groups, showHeadings: groups.length >= 2 };",
    replace: "  return { groups, showHeadings: true };",
  },
  {
    id: "pad/take-payment-primary-at-a-table",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — at a table the Send is the primary and Take payment secondary. Primary, it is a second filled pill beside the Send and makes paying the hero before the food is sent",
    find: '  const variant = i.mode === "dinein" && i.open ? "secondary" : "primary";',
    replace: '  const variant = "primary" as "primary" | "secondary";',
  },
  {
    id: "pad/take-payment-secondary-at-the-counter",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — a counter order has no Send, so Take payment IS its one primary. Gated on `open` alone it is secondary on the only action the screen has",
    find: '  const variant = i.mode === "dinein" && i.open ? "secondary" : "primary";',
    replace: '  const variant = i.open ? "secondary" : "primary";',
  },
  {
    id: "pad/unknown-add-does-not-hold-payment",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — an add whose fate is unknown holds Take payment. Released, the cashier takes payment while a dish may or may not be on the bill",
    find: "    : i.pending.unconfirmed + i.pending.lost > 0 || i.sendBusy",
    replace: "    : i.sendBusy",
  },
  {
    id: "pad/flying-add-refuses-take-payment",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — a tap while an add FLIES is accepted (the pad drains it). Refused, a cashier waits out every round trip before paying",
    find: "      : i.itemCount === 0 && inFlight === 0",
    replace: "      : i.itemCount === 0 || inFlight > 0",
  },
  {
    id: "pad/paying-not-first",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — a guest paying outranks every other reason: it is the one the cashier can act on at the table",
    find: '  const block: PadSettleBlock | null = i.paying\n    ? "paying"',
    replace:
      '  const block: PadSettleBlock | null =\n    i.paying && i.pending.lost === 0\n    ? "paying"',
  },
  {
    id: "pad/amount-over-an-unseen-add",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — no amount is named while an add is pending. Dropping `unseen`, the subtotal and Take payment name a sum a landed-but-unread dish already changed (§23)",
    find: "  const adds = p.flying + p.unseen + p.unconfirmed + p.lost === 0;",
    replace: "  const adds = p.flying + p.unconfirmed + p.lost === 0;",
  },
  {
    id: "pad/count-on-a-landed-add",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — the Send is bare while any add is unread. A count from flying adds only claims a number the landed dish has already moved",
    find: "  const bare = i.pending.flying + i.pending.unseen > 0;",
    replace: "  const bare = i.pending.flying > 0;",
  },
  {
    id: "pad/no-send-for-the-first-dish",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — the first dish in flight on an empty table still offers a (bare) Send. Passing the server's `none` through, a fast 'add, send' lands on nothing",
    find: '  if (!bare || view.kind === "send") return { view, bare };',
    replace: "  return { view, bare };",
  },
  {
    id: "pad/lost-add-blocks-the-tiles",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — only an UNCONFIRMED add holds the tiles (the hung request queues every later one). A LOST add answered: blocking the tiles on it stalls a whole order on one dish",
    find: '  if (i.pending.unconfirmed > 0) return "waiting";',
    replace: '  if (i.pending.unconfirmed + i.pending.lost > 0) return "waiting";',
  },
  {
    id: "pad/hung-add-leaves-tiles-live",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — a hung add holds the tiles: Next queues every later action behind it, so a tap would sit silently in the queue",
    find: '  if (i.pending.unconfirmed > 0) return "waiting";\n',
    replace: "",
  },
  {
    id: "pad/commit-drops-every-landed-ghost",
    file: "apps/qr/lib/pad-pending.ts",
    suite: "lib/pad-pending.test.ts",
    why: "Phase 2c · pad — a landed ghost leaves only on a read that STARTED after it landed. Dropped on any commit, a poll already in the air shows the order without the dish and the ghost vanishes with nothing in its place",
    find: '        (p) => !(p.state === "landed" && p.landedSeq !== null && p.landedSeq < e.readStartSeq),',
    replace: '        (p) => !(p.state === "landed"),',
  },
  {
    id: "pad/commit-drops-at-the-landing-read",
    file: "apps/qr/lib/pad-pending.ts",
    suite: "lib/pad-pending.test.ts",
    why: "Phase 2c · pad — the read that was the LAST started when the add landed may have begun before the write; only a strictly later start drops the ghost",
    find: '        (p) => !(p.state === "landed" && p.landedSeq !== null && p.landedSeq < e.readStartSeq),',
    replace:
      '        (p) => !(p.state === "landed" && p.landedSeq !== null && p.landedSeq <= e.readStartSeq),',
  },
  {
    id: "pad/refusal-removes-the-dish",
    file: "apps/qr/lib/pad-pending.ts",
    suite: "lib/pad-pending.test.ts",
    why: "Phase 2c · pad — a refusal is removed by its ADD KEY. By dish, a second Mohinga still on its way vanishes from the ticket while it may yet land on the bill",
    find: "      return s.filter((p) => p.key !== e.key);",
    replace: "      return s.filter((p) => p.itemId !== s.find((x) => x.key === e.key)?.itemId);",
  },
  {
    id: "pad/retry-mints-a-new-attempt",
    file: "apps/qr/lib/pad-pending.ts",
    suite: "lib/pad-pending.test.ts",
    why: "Phase 2c · pad — Send again flies the SAME key. A fresh attempt is a new key, and the ledger cannot dedupe it: a second plate cooked and charged",
    find: '      return move(s, e.key, ["lost"], (p) => ({ ...p, state: "flying" }));',
    replace:
      '      return move(s, e.key, ["lost"], (p) => ({ ...p, key: `${p.key}-again`, state: "flying" }));',
  },
  {
    id: "pad/timeout-moves-a-landed-add",
    file: "apps/qr/lib/pad-pending.ts",
    suite: "lib/pad-pending.test.ts",
    why: "Phase 2c · pad — 15s with no answer moves only a FLYING add. A landed one reading 'Checking…' holds the Send and Take payment over a dish that is on the order",
    find: '      return move(s, e.key, ["flying"], (p) => ({ ...p, state: "unconfirmed" }));',
    replace:
      '      return move(s, e.key, ["flying", "landed"], (p) => ({ ...p, state: "unconfirmed" }));',
  },
  {
    id: "pad/unseen-counted-as-flying",
    file: "apps/qr/lib/pad-pending.ts",
    suite: "lib/pad-pending.test.ts",
    why: "Phase 2c · pad — landed-but-unread is its own count. Folded into flying, the Take payment drain waits on an add that already answered",
    find: '    else if (p.state === "landed") c.unseen += 1;',
    replace: '    else if (p.state === "landed") c.flying += 1;',
  },
  {
    id: "pad/a-lost-add-is-no-blocker",
    file: "apps/qr/lib/pad-pending.ts",
    suite: "lib/pad-pending.test.ts",
    why: "Phase 2c · pad — a LOST add holds the Send and Take payment until it is resent or reloaded: it may be on the bill",
    find: '  return s.find((p) => p.state === "unconfirmed" || p.state === "lost") ?? null;',
    replace: '  return s.find((p) => p.state === "unconfirmed") ?? null;',
  },
  {
    id: "pad/unconfirmed-read-as-a-refusal",
    file: "apps/qr/lib/pad-errors.ts",
    suite: "lib/pad-errors.test.ts",
    why: "Phase 2c · pad — a write that may have committed is UNKNOWN. Read as a refusal the ghost vanishes, the region says 'didn't go on', and the re-tap mints a NEW key: a second plate",
    find: '  if (outcome === "unknown") return { kind: "unknown" };',
    replace:
      '  if (outcome === "unknown" && res === "threw") return { kind: "unknown" };\n  if (outcome === "unknown")\n    return { kind: "refused", err: { kind: "key", key: ADD_ERR.failed.key, code: "unconfirmed" } };',
  },
  {
    id: "pad/threw-read-as-a-refusal",
    file: "apps/qr/lib/pad-errors.ts",
    suite: "lib/pad-errors.test.ts",
    why: "Phase 2c · pad — an action that threw lost its answer; the add may have landed. Read as a refusal it invites the double add the key exists to prevent",
    find: '  if (outcome === "unknown") return { kind: "unknown" };',
    replace:
      '  if (res === "threw")\n    return { kind: "refused", err: { kind: "key", key: ADD_ERR.failed.key, code: undefined } };\n  if (outcome === "unknown") return { kind: "unknown" };',
  },
  {
    id: "pad/sold-out-collapsed-into-failed",
    file: "apps/qr/lib/pad-errors.ts",
    suite: "lib/pad-errors.test.ts",
    why: "Phase 2c · pad — a dish that just sold out says so (and the tile turns). Collapsed into 'try again', staff retry a dish nobody can make",
    find: '  sold_out: "soldOut",',
    replace: '  sold_out: "failed",',
  },
  {
    id: "pad/a-correction-without-its-family",
    file: "apps/qr/lib/pad-errors.ts",
    suite: "lib/pad-errors.test.ts",
    why: "Phase 2c · pad — two dishes refused for one cause become the family sentence. Without it the second erases the first dish's retraction and its claim stands unretracted",
    find: '  return padSlotNotice("correction", key, { x: dish }, { family: FAMILY.get(key) });',
    replace: '  return padSlotNotice("correction", key, { x: dish });',
  },
  {
    id: "pad/a-send-refusal-read-as-news",
    file: "apps/qr/lib/pad-errors.ts",
    suite: "lib/pad-errors.test.ts",
    why: "Phase 2c · pad — a refused send is a CORRECTION. As news it can be erased by the next claim before anyone reads why the order did not go",
    find: '  const kind: NoticeKind = n.tone === "ok" ? "news" : "correction";',
    replace: '  const kind: NoticeKind = "news";',
  },
  {
    id: "floor/pad-line-loses-its-dish",
    file: "apps/qr/lib/floor.ts",
    suite: "lib/floor-pad-lines.test.ts",
    why: "Phase 2c · pad — the line carries its dish; the tile's confirmed ×N counts by it. Dropped, no tile ever shows a dish is on the order",
    find: "      menuItemId: i.menu_item_id ?? null,",
    replace: "      menuItemId: null,",
  },
  {
    id: "floor/pad-togo-read-as-dinein",
    file: "apps/qr/lib/floor.ts",
    suite: "lib/floor-pad-lines.test.ts",
    why: "Phase 2c · pad — a to-go draft reads to-go on the pad's ticket. Forced to dine-in it lands under 'Not sent yet' and staff send a dish the Send will not fire",
    find: '      fulfillment:\n        i.fulfillment === "togo" || i.fulfillment === "grocery"\n          ? i.fulfillment\n          : ("dinein" as const),\n',
    replace: '      fulfillment: "dinein" as const,\n',
  },
  {
    id: "floor/pad-name-my-unvalidated",
    file: "apps/qr/lib/floor.ts",
    suite: "lib/floor-pad-lines.test.ts",
    why: "Phase 2c · pad — a line's Burmese is a catalog fact or nothing. Raw, a romanised name_my leads the ticket as 'Burmese'",
    find: "      nameMy: catalogNameMy(i.menu_item_id ? nameMyById.get(i.menu_item_id) : null, i.name),",
    replace: "      nameMy: i.menu_item_id ? (nameMyById.get(i.menu_item_id) ?? null) : null,",
  },
  {
    id: "floor/pad-options-lose-their-ids",
    file: "apps/qr/lib/floor.ts",
    suite: "lib/floor-pad-lines.test.ts",
    why: "Phase 2c · pad — the chosen options' Burmese pairs by `modifier_option_ids`. Unread, every option renders English on a Burmese console",
    find: '        "id,name,qty,unit_price_cents,by_seat,created_at,menu_item_id,state,comped,notes,modifiers,fulfillment,modifier_option_ids",\n',
    replace:
      '        "id,name,qty,unit_price_cents,by_seat,created_at,menu_item_id,state,comped,notes,modifiers,fulfillment",\n',
  },
  // The critic round: the note Take payment waits on, the ticket's own writes, the reasons.
  {
    id: "pad/take-payment-blind-to-the-note",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — an unsaved kitchen note holds Take payment. Blind to it, the pad navigates, the editor unmounts, and a walk-up's 'no peanuts' is thrown away — a counter order cooks when paid with no note",
    find: '    : i.unsavedNote\n      ? "note"\n      : ',
    replace: "    : ",
  },
  {
    id: "pad/note-hold-reads-sendable-only",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — Take payment waits on ANY line's unsaved note. Reading only what the Send fires, a counter order's lines (never sendable) and a table's to-go drafts lose their note with nothing guarding it",
    find: "  const note = edits.find((e) => e.noteDirty);",
    replace: "  const note = edits.find((e) => e.sendable && e.noteDirty);",
  },
  {
    id: "pad/line-write-leaves-payment-live",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — a qty change or a removal still saving holds Take payment. Released, the table page opens on a total the write is about to move",
    find: " || i.sendBusy || i.lines.writing > 0",
    replace: " || i.sendBusy",
  },
  {
    id: "pad/amount-over-a-line-write",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — no amount beside a line write in flight. Named, 'Take payment · $15.81' sits beside a quantity just changed to 3, or a row already leaving (§23)",
    find: "  return adds && lines.writing === 0 && !lines.unread;",
    replace: "  return adds && !lines.unread;",
  },
  {
    id: "pad/amount-over-an-unread-line",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — a line write that answered is unread until a later read commits. Shown at the answer, the old total is named beside the new quantity",
    find: "  return adds && lines.writing === 0 && !lines.unread;",
    replace: "  return adds && lines.writing === 0;",
  },
  {
    id: "pad/busy-always-waits-for-a-dish",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — Take payment waits for a dish only while one is on its way. Always 'Waiting for the last dish…', it claims a dish that is not coming",
    find: '  return i.flying > 0 ? "draining" : i.saveName ? "saving" : "opening";',
    replace: '  return "draining";',
  },
  {
    id: "pad/chip-never-toggles-off",
    file: "apps/qr/lib/order-pad.ts",
    suite: "lib/order-pad.test.ts",
    why: "Phase 2c · pad — tapping the lit chip goes back to All. Never toggling, the only way back to the whole menu is the All chip",
    find: "  return slug === null || activeCat === slug ? null : slug;",
    replace: "  return slug;",
  },
  {
    id: "pad/unread-cleared-at-the-landing-read",
    file: "apps/qr/lib/pad-pending.ts",
    suite: "lib/pad-pending.test.ts",
    why: "Phase 2c · pad — a line write's figures clear only on a read that STARTED after it answered. At the landing read, a poll already in the air names the old total beside the new quantity",
    find: "  return unreadSeq !== null && unreadSeq < readStartSeq ? null : unreadSeq;",
    replace: "  return unreadSeq !== null && unreadSeq <= readStartSeq ? null : unreadSeq;",
  },
  {
    id: "pad/lost-add-worded-as-a-wait",
    file: "apps/qr/lib/staff-send-view.ts",
    suite: "lib/staff-send-view.test.ts",
    why: "Phase 2c · pad — a LOST add names the fix (Try again, or reload). Worded 'Waiting to hear back', nothing is coming and a family member waits forever",
    find: '        k: hold.state === "lost" ? "table.send.hold.lost" : "table.send.hold.add",',
    replace: '        k: "table.send.hold.add",',
  },
  {
    id: "pad/a-hold-outranks-a-paying-guest",
    file: "apps/qr/lib/staff-send-view.ts",
    suite: "lib/staff-send-view.test.ts",
    why: "Phase 2c · pad — a payment holding the cart is the reason a refused Send gives first. Under a hold it tells staff to save a note while the Send cannot go anyway",
    find: '  if (view.blocked === "paying") return { k: "table.send.paying" };\n',
    replace: "",
  },
  {
    id: "pad/sheet-unconfirmed-releases-the-key",
    file: "apps/qr/lib/pad-errors.ts",
    suite: "lib/pad-errors.test.ts",
