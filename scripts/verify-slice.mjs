...
const MUTANTS = [
  ...
  },
  {
    ...
    replace: "  const landed = grew.reduce((a, d) => a + d, 0);",
  },
  {
  ...
  },
  {
    ...
    file: "apps/qr/lib/add-landing.ts",
    suite: "lib/add-landing.test.ts",
    why: "T21(c) \u2014 when NOTHING of ours grew and a line of this dish shrank, our add is invisible in the difference and the cap is NOT established: a peer removed units inside our round trip. Answering `none` hands it to `partialAddNotice`, which says the line is already at our 99 max \u2014 about a dish that just got smaller, and about a write we never confirmed. `unknown` exists so the one state we cannot explain stays unexplained",
    find: '  if (landed === 0) return { landed: 0, outcome: shrank ? "unknown" : "none" };',
    replace: '  if (landed === 0) return { landed: 0, outcome: "none" };',
  },
  {
