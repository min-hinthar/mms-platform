...
const MUTANTS = [
  ...
  {
    id: "landing/units-counted-basket-wide",
    file: "apps/qr/lib/add-landing.ts",
    suite: "lib/add-landing.test.ts",
    why: 'T21(c) \u2014 THIS IS THE SHIPPED SUCCESS-PATH DEFECT, restored. Subtracting basket-wide totals lets ANY concurrent peer write skew the count: a tablemate removing one unit of THEIR line makes `landed` come out one short of the request and fires "Added 4 \u2014 that line is now at our 99 max" about a dish sitting at 6 of 99. It needs no near-cap line at all, so it is likelier than the cap it claims, and it is a fabricated diagnosis spoken into the live region',
    find: "  rows.reduce((a, r) => a + (r.menuItemId === menuItemId ? r.qty : 0), 0);",
    replace: "  rows.reduce((a, r) => a + r.qty, 0);",
  },
  {
    ...
    find: '  return { landed, outcome: landed >= input.requested ? "full" : "partial" };',
  ...
  },
  {
    ...
    file: "apps/qr/lib/add-landing.ts",
    suite: "lib/add-landing.test.ts",
    why: "T21(c) \u2014 a count that went DOWN describes the TABLE, not this tap: a peer removed units of the same dish inside our round trip. Collapsing that into `none` hands it to `partialAddNotice`, which says the line is already at our 99 max \u2014 about a dish that just got smaller. `unknown` exists so the one state we cannot explain stays unexplained",
    find: '  if (landed < 0) return { landed, outcome: "unknown" };',
    replace: '  if (landed < 0) return { landed, outcome: "none" };',
  },
  {
