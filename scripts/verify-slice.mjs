...
const MUTANTS = [
  ...
  {
    id: "pulse/ready-linger-unbounded",
    file: "apps/qr/lib/board-pulse.ts",
    suite: "lib/board-pulse.test.ts",
    why: "P6 — nothing records whether a runner carried the plate out, so `ready` is only honest inside a bounded window after `bumped_at`. Unbounded, a table reads `ready` from the bump until it pays — half an hour of a public screen asserting a state nobody checked",
    find: "      if (Number.isFinite(bumpedMs) && bumpedMs >= readyFloorMs) readySessions.add(cart.session_id);",
    replace: "      if (Number.isFinite(bumpedMs)) readySessions.add(cart.session_id);",
  },
  {
    id: "pulse/cooking-loses-to-ready",
    file: "apps/qr/lib/board-pulse.ts",
    suite: "lib/board-pulse.test.ts",
    why: "P6 — one table number can carry a just-bumped first course and an already-fired second (or two sessions across a re-seat). `ready` winning sends a runner to the pass for a plate that is still on the wok; `cooking` is the safe reading and must not depend on which row the map saw first",
    find: '    const next: PulseTableStatus = cooking ? "cooking" : "ready";',
    replace: '    const next: PulseTableStatus = ready ? "ready" : "cooking";',
  },
  {
