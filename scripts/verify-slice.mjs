...
const MUTANTS = [
  ...
  },
  {
    ...
    why: "M186, and it is the exact line that shipped. Refreshing the stamp only when the gate EMITS makes the window measure time since the last SCAN rather than time since the barcode was last SEEN \u2014 so one item resting in front of the camera re-opens it every 1500ms, mints a fresh `scanId`, and the server counts each as a deliberate re-scan. Ten seconds of natural dwell is six charges",
    find: "  const next: ScanGate = { code, seenAt: now };",
    replace: "  const next: ScanGate = { code, seenAt: gate.code === code ? gate.seenAt : now };",
  },
  {
    id: "m186/a-re-present-stops-counting",
    file: "apps/qr/lib/scan-gate.ts",
    suite: "lib/scan-gate.test.ts",
    why: "The other direction, and the reason the window exists at all: showing the same item again after a real gap is how a diner adds a second one with the camera. Collapse the gap test and the gate silently eats every repeat purchase \u2014 a refusal to charge is still a wrong number, and this one is invisible until the receipt",
    find: "  if (now - gate.seenAt >= quietMs) return { emit: true, next };",
  ...
  },
  {
    ...
    file: "apps/qr/lib/scan-gate.ts",
    suite: "lib/scan-gate.test.ts",
    why: "Two different items in quick succession is the normal shopping motion. Fold the identity test into the timer and the second item is swallowed for a second and a half, which reads to the diner as the scanner having missed it \u2014 so they present it again, and now the timing decides whether they are charged once or twice",
