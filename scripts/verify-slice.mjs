...
const MUTANTS = [
  ...
  },
  {
    ...
    find: "    const v = seen.view;\n    if (!v) return null;",
    replace: '    const v = seen.view;\n    if (!v) return { cause: "unknown" };',
  },
  {
  ...
  {
    id: "t33/cart-latch-forgets-whose-lock-it-is",
    file: "apps/qr/components/Checkout.tsx",
    suite: "components/Checkout.test.tsx",
    why: "T33's ownership half, on /cart. `explanationHolds` compares `current.lockedByYou` against the refusal's own attribution, so a hardcoded value at latch time silently drops the SELF-held lock's sentence \u2014 the two-tab case `freezeNotice` names \u2014 while the peer twin stays green. Through `cartFreeze`, the same function that gave the refusal its attribution, so the latch and the sentence fork on ONE fact",
    find: '      lockedByYou: cartFreeze(f) === "self",',
    replace: "      lockedByYou: false,",
  },
  {
    ...
    find: '    if (held.axis === "locked" && !locked) explainedFreezeRef.current = null;',
    replace: "",
  },
  {
