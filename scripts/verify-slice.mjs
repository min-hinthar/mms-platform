...
const MUTANTS = [
  ...
  },
  {
    ...
    find: '  if (outcome === "captured") return "paying";',
    replace: '  if (outcome === "captured") return "locked";',
  },
  {
    id: "settle/unreachable-stripe-reads-as-a-diner",
    file: "apps/qr/lib/supersede.ts",
    suite: "lib/settle-takeover.test.ts",
    why: "M119's rule one hop out: a transport failure is not a verdict. `unknown` means we could not reach Stripe to judge the abandoned attempt, and answering `locked` tells staff a diner is checking out \u2014 a fact nobody established, on the screen they read to decide what to do next",
    find: '  if (outcome === "unknown") return "unavailable";',
    replace: '  if (outcome === "unknown") return "locked";',
  },
  {
