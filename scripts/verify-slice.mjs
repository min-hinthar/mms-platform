...
const MUTANTS = [
  ...
  {
    id: "reorder/unreadable-availability-empties-the-menu",
    file: "apps/qr/lib/reorder.ts",
    suite: "lib/reorder.test.ts",
    why: "M119 (e) — an EMPTY `itemById` does not mean nothing is available, it means we never asked. Unbound, a failed availability read skips every food line as `gone` and hands the diner an empty cart plus one false statement per dish (`isn't available today`). Refusing is the conservative direction: the alternative skips the availability check entirely and re-adds a delisted or sold-out dish, which is what W23a's server-side half exists to prevent",
    find: "  if (itemsError)",
    replace: "  if (false)",
  },
  {
