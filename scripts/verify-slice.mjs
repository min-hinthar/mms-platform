...
const MUTANTS = [
  ...
  {
    id: "settle/cancels-whatever-the-row-names-now",
    file: "apps/qr/lib/supersede.ts",
    suite: "lib/settle-takeover.test.ts",
    why: "The other half of the same P1, and a one-word edit. Passing the CART re-reads the link inside `supersedeCartIntent`, so even with the claim held the cancel targets whatever the row names at that instant rather than the attempt we diagnosed and claimed against. Naming the id is what makes the claim mean anything",
    find: "    const outcome = await supersede(live, classifyLiveIntentForSettlement);",
    replace: "    const outcome = await supersede(cartId, classifyLiveIntentForSettlement);",
  },
  {
