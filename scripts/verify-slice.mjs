...
const MUTANTS = [
  ...
  },
  {
    ...
    find: '    if (claimHeld) await releaseOwn(cartId, owner, "post-claim-throw");',
    replace: "    if (claimHeld) void owner;",
  },
  {
  ...
  {
    id: "settle/refusal-strands-the-claimed-freeze",
    file: "apps/qr/lib/supersede.ts",
    suite: "lib/settle-takeover.test.ts",
    why: "A3 · M201 — same as `throw-after-claim-strands-the-freeze`, on the refused-supersede exit: a refused takeover must give the freeze back under its request-unique owner, or every tender on the table waits out the settle TTL for an attempt we decided not to touch. Held-to-the-TTL was the cost of a shared owner, never a doctrine",
    find: '      await releaseOwn(cartId, owner, "supersede-refused");',
    replace: "      void owner;",
  },
  {
  ...
  },
  {
    ...
    find: "    const refreezeErr = released ? await refreeze(db, id, uid) : null;",
