...
const MUTANTS = [
  ...
  },
  {
    ...
    find: '    : settled.gte("created_at", sinceIso)',
    replace: "    : settled",
  },
  {
    id: "refunds/the-union-arm-drops-the-floor",
    file: "apps/qr/lib/refunds.ts",
    suite: "lib/refunds.test.ts",
    why: "A4·3 (blind pass, CRITICAL 1) — once any refund exists today the read takes the union arm, and that arm must still floor the PAID orders: dropping `created_at.gte` from the `or` lists every settled order ever alongside today's refunds, on exactly the days a manager is reconciling a refund",
    find: '    ? settled.or(`created_at.gte.${sinceIso},id.in.(${refundedTodayIds.join(",")})`)',
    replace: '    ? settled.or(`id.in.(${refundedTodayIds.join(",")})`)',
  },
  {
