...
const MUTANTS = [
  ...
  },
  {
    ...
    file: "apps/qr/components/TableCartProvider.tsx",
    suite: "components/TableCartProvider.test.tsx",
    why: "M193 \u2014 THE SHIPPED DEFECT, restored. One add writes to BOTH watched tables (the `qr_cart_items` INSERT and `touchCart`'s `qr_carts` UPDATE), so a single tap echoed back at least twice and each echo ran its own `getCartView`: ~7 sequential DB round trips apiece. Measured in production 2026-09-09: four `POST /cart` inside four seconds from ONE tap, fanned out to every phone at the table. The un-coalesced form is behaviourally correct \u2014 it re-reads MORE, never less \u2014 so only a call-count assertion falsifies it",
    find: "      if (echoSince.current === null) echoSince.current = Date.now();\n      const waited = Date.now() - echoSince.current;\n      const delay = Math.max(0, Math.min(ECHO_COALESCE_MS, ECHO_MAX_WAIT_MS - waited));\n      if (echoTimer.current) clearTimeout(echoTimer.current);\n      echoTimer.current = setTimeout(() => {\n        echoTimer.current = null;\n        echoSince.current = null; // the burst is over; the next event starts a fresh deadline\n        void refresh();\n      }, delay);",
  ...
  {
    id: "settle/throw-after-claim-keeps-the-freeze",
    file: "apps/qr/lib/supersede.ts",
    suite: "lib/settle-takeover.test.ts",
    why: "Codex rounds 8/10/11/12/13 on #275. Releasing this freeze is the OBVIOUS change and every form of it was falsified: by OWNER (staff-cart passes caller.uid, so a same-staff sibling's row is byte-identical \u2014 shipped in bebeef8, reverted in bd362b6), by owner+ERA (the era says who WROTE the freeze, not who DEPENDS on it: create-share-intent mints at :190 and only calls extendSettlement at :328, so it rides a freeze it never wrote, and clearing it hands back a confirmable client secret with NO mutex), and by CART (matches everyone). The freeze is HELD to the settle TTL until M201 gives it a request-unique owner. This mutant IS the re-addition, so it cannot come back by accident",
    find: '    if (claimHeld) logHeldFreeze(cartId, "post-claim-throw");',
  ...
  },
  {
    ...
    find: '      logHeldFreeze(cartId, "pin-clear-failed");\n      return "unavailable";\n    }\n    // The freeze is already ours',
  ...
  },
  {
    ...
    find: "  const err = await releaseSettlementFor(cartId, probe);",
  ...
  },
  {
    ...
    file: "apps/qr/lib/supersede.ts",
    suite: "lib/settle-takeover.test.ts",
    why: "The probe's uniqueness is load-bearing TWICE. `releaseSettlementFor` is scoped by `settle_by`, so only a uuid nobody else can hold makes the release provably ours \u2014 under the caller's uid it could null the WINNER's freeze, the very request we stood down for. And a unique owner cannot match `acquireSettlement`'s `settle_by.eq.<uid>` arm, which is what makes the promotion structurally impossible rather than suppressed after the fact",
  ...
  },
  {
    ...
    file: "apps/qr/lib/settle-release-scope.ts",
    suite: "lib/settle-release-scope.test.ts",
    why: "`closedByStaffId` is WHO GETS CREDIT for the close; the freeze is held under `caller.uid`. Falling back to it looks like resilience and is the silent-no-op failure: the scoped release would match zero rows forever, so the freeze it exists to return would never come back and every table healed only on the TTL \u2014 a bug that looks exactly like working code",
    find: "  const uid = meta.closedByUid;",
    replace: "  const uid = meta.closedByUid ?? meta.closedByStaffId;",
  },
  {
  ...
  {
    id: "settle/ambiguous-claim-released-under-a-shared-owner",
    file: "apps/qr/lib/supersede.ts",
    suite: "lib/settle-takeover.test.ts",
    why: "Codex round 11 on #275, P1 \u2014 the REGRESSION I shipped for round 10, as a mutant so it cannot come back. Cleaning up an ambiguous claim looks obviously right and is unsafe: `uid` is NOT request-unique (`staff-cart.ts` passes `caller.uid` at both call sites; only `terminal.ts` passes a per-attempt id), so a release from request B whose claim errored matches same-staff request A's LIVE claim and strips A's mutex while A is charging. That trades a self-healing 10-minute TTL freeze for an unprotected concurrent settle. Undecidable from here \u2014 A's row and ours are byte-identical. The fix is a request-unique owner, M201",
    find: '        ambiguous: true,\n      });\n      return "unavailable";',
    replace:
  ...
  {
    id: "settle/refusal-keeps-the-claimed-freeze",
    file: "apps/qr/lib/supersede.ts",
    suite: "lib/settle-takeover.test.ts",
    why: "Codex rounds 8/10/11/12/13 on #275. Releasing this freeze is the OBVIOUS change and every form of it was falsified: by OWNER (staff-cart passes caller.uid, so a same-staff sibling's row is byte-identical \u2014 shipped in bebeef8, reverted in bd362b6), by owner+ERA (the era says who WROTE the freeze, not who DEPENDS on it: create-share-intent mints at :190 and only calls extendSettlement at :328, so it rides a freeze it never wrote, and clearing it hands back a confirmable client secret with NO mutex), and by CART (matches everyone). The freeze is HELD to the settle TTL until M201 gives it a request-unique owner. This mutant IS the re-addition, so it cannot come back by accident",
    find: '      logHeldFreeze(cartId, "supersede-refused");',
    replace: "      await releaseSettlementFor(cartId, uid);",
  },
  {
  ...
  },
  {
    ...
    find: "  if (error) return { ok: true, role: caller.role };",
    replace: "  if (error) return { ok: false };",
  },
  {
  ...
  },
  {
    ...
    find: '  if (!roleAtLeast(role, "manager")) return { ok: false };\n  return { ok: true, role };',
    replace:
