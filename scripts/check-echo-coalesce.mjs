 ...
 * deliberately does not load — it runs in the fast lane in ~1.7 s over the whole app. Both are
 ...
 *
 * Round 9 added a fifth, and it is a REACH gap rather than a resolution one: a BARREL RE-EXPORT
 * (`export { useCartRealtime } from "./realtime";`) hides a consumer entirely, because
 * `importedFrom` resolves the importing file's specifier to the BARREL and not to `REALTIME_MODULE`
 * — the call site is never collected and `MIN_CALL_SITES` stays satisfied by the two direct
 * imports. Filed as M226(h); the cheap fix, if it is ever picked up, is to refuse any re-export of
 * the hook rather than to trace chains, because that is fail-loud and small.
 *
 ...
 * slower job, not a step that parses the whole app in ~1.7 s.
 *
...
/**
 * Source files, EXCLUDING `.d.ts`.
 *
 * ⚠️ A DECLARATION FILE CANNOT HOLD A CALL SITE OR A VALUE — and counting one made the scanned-file
 * TOTAL depend on whether anyone had run a build. `apps/qr/next-env.d.ts` is generated by
 * `next build` and gitignored, so the guard printed 679 on a developed tree and 678 in a fresh
 * checkout — and in CI's own fast lane, which runs BEFORE the build job. Codex round 9 on #287
 * measured 678 and read the gap as a leftover review fixture; the tree was clean (git-verified),
 * and the generated file was the whole difference. A number that is a property of the OPERATOR
 * rather than the repo cannot be transcribed anywhere, so the scan is made deterministic here and
 * the prose stopped quoting it.
 */
const SOURCE_EXT = /(?<!\.d)\.tsx?$|\.mjs$/;
...
const windowFiles = [...new Set(WINDOW_SCAN_ROOTS.flatMap((d) => sources(d, [], SOURCE_EXT)))];
