...
- **The CI fast lane grew teeth** (T6 · M149). `format:check`, `check:migration-versions` and
  ...
  now THIRTEEN steps and the list above is only the first six** — measure it, never read it off this
  ...
  `check:staff-lang`, `check:child-freeze`, `check:echo-coalesce` and `check:mutant-anchors` (the last of which answers in
  ...
  `bash -e`, so a stale doc count is not cosmetic — it stops the TWELVE guards behind it, the orphan
...
pnpm verify:slice        # the MECHANICAL pre-PR gate: money-path coverage guard (a changed money
                         ...
                         # + photo-filter grep + gate + 731 mutations + orphan check (a few minutes)
