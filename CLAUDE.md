...
- **The CI fast lane grew teeth** (T6 · M149). `format:check`, `check:migration-versions` and
  ...
  now FIFTEEN steps and the list above is only the first six** — measure it, never read it off this
  ...
  it — and the one step it hid, `node scripts/check-test-env.mjs` (ci.yml:164), is the guard that
  ...
  ~1s what `verify:slice` only reports 20 minutes into a full run — LEARNINGS #106) and `check:scan-repeat`
  (M186's wiring: the charge RULE is a mutated lib module, but the four-line guard clause that CALLS
  it lives in a component no suite and no `MONEY_PATHS` rule can see). Phase 0 added the fifteenth, `check:style-literals` — the weight/size/tracking ratchet (a hardcoded count may fall, never rise; re-record with `--update`). ⚠️ `check:docs` is step ONE and GitHub's shell is
  `bash -e`, so a stale doc count is not cosmetic — it stops the FOURTEEN guards behind it, the orphan
...
pnpm verify:slice        # the MECHANICAL pre-PR gate: money-path coverage guard (a changed money
                         ...
                         # + photo-filter grep + gate + 1108 mutations + orphan check (a few minutes)
                         # ⚠️ REWRITES the 163 money/authority modules it mutates IN PLACE (138 under
                         # apps/qr/lib, plus create-share-intent, setup-intent and board routes,
                         # plus TWENTY-ONE components — TableCartProvider.tsx, menu/YourUsual.tsx,
                         # staff/TicketText.tsx, staff/Chrome.tsx, staff/StaffPromoControl.tsx,
                         ...
                         # staff/OrderPad.tsx (Phase 2c), and three HOOKS under components/ —
                         # staff/usePadWrites.ts, staff/useStaffSend.ts and staff/usePadDetailLive.ts
                         # (Phase 2c review fixes) — COMPONENTS in the set since M46 — plus
                         # packages/db/src/schemas.ts) and restores them.
                         # ⚠️ 138+3+21+1=163, and THIS ENUMERATION IS THE OPERATOR'S ONLY LIST of files
