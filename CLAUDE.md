...
pnpm verify:slice        # the MECHANICAL pre-PR gate: money-path coverage guard (a changed money
                         ...
                         # + photo-filter grep + gate + 787 mutations + orphan check (a few minutes)
                         # ⚠️ REWRITES the 139 money/authority modules it mutates IN PLACE (122 under
                         # apps/qr/lib, plus create-share-intent, setup-intent and board routes,
                         # plus THIRTEEN components — TableCartProvider.tsx, menu/YourUsual.tsx,
                         ...
                         # Checkout.tsx (M224) and PaymentSection.tsx (Phase 1c), COMPONENTS in
                         # the set since M46 — plus
                         # packages/db/src/schemas.ts) and restores them.
                         # ⚠️ 122+3+13+1=139, and THIS ENUMERATION IS THE OPERATOR'S ONLY LIST of files
