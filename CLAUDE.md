...
pnpm verify:slice        # the MECHANICAL pre-PR gate: money-path coverage guard (a changed money
                         ...
                         # + photo-filter grep + gate + 379 mutations + orphan check (a few minutes)
                         # ⚠️ REWRITES the 78 money/authority modules it mutates IN PLACE (70 under
                         ...
                         # staff/TicketText.tsx, staff/Chrome.tsx and ReadyBoard.tsx, the first
                         # COMPONENTS in the set, since M46) and restores them.
                         # ⚠️ 70+3+5=78, and this list is the operator's ONLY inventory of files
                         # that may be sitting on disk as deliberately-broken mutants after a
                         # stalled or killed run (LEARNINGS #74), so a bucket count that does not
                         # add up is an operational hazard, not a typo. It read FOUR while the set
                         # held five. MEASURE the buckets, never count them by eye:
                         #   grep -oE '^\s+file: "[^"]+"' scripts/verify-slice.mjs | sort -u \
                         #     | cut -d'"' -f2 | sed -E 's#(apps/qr/(lib|app/api))/.*#\1#' | sort | uniq -c
