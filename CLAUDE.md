...
pnpm verify:slice        # the MECHANICAL pre-PR gate: money-path coverage guard (a changed money
                         ...
                         # + photo-filter grep + gate + 414 mutations + orphan check (a few minutes)
                         # ⚠️ REWRITES the 83 money/authority modules it mutates IN PLACE (74 under
                         # apps/qr/lib, plus create-share-intent, setup-intent and board routes, plus
                         # FIVE components — TableCartProvider.tsx, menu/YourUsual.tsx,
                         # staff/TicketText.tsx, staff/Chrome.tsx and staff/StaffPromoControl.tsx,
                         # COMPONENTS in the set since M46) and restores them. ⚠️ THIS ENUMERATION
                         # IS THE OPERATOR'S ONLY LIST of files that may be sitting on disk as
                         # deliberately-broken mutants after a stalled or killed run — the reason
                         # LEARNINGS #74 exists and `git checkout -- .` is prescribed — so a PR that
                         # adds a component to the mutate set updates it HERE, not just the totals.
                         # Measure, never count by eye:
                         #   grep -oE '^\s+file: "[^"]+"' scripts/verify-slice.mjs | sort -u \
                         #     | grep -c components/ It ABORTS if a target file is DIRTY — commit or stash
