...
pnpm verify:slice        # the MECHANICAL pre-PR gate: money-path coverage guard (a changed money
                         ...
                         # + photo-filter grep + gate + 215 mutations + orphan check (a few minutes)
                         # ⚠️ REWRITES the 54 money/authority modules it mutates IN PLACE (52 under
                         # apps/qr/lib, plus create-share-intent/route.ts and board/route.ts) and
