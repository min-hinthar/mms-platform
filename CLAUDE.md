...
pnpm verify:slice        # the MECHANICAL pre-PR gate: money-path coverage guard (a changed money
                         ...
                         # + photo-filter grep + gate + 578 mutations + orphan check (a few minutes)
                         # ⚠️ REWRITES the 108 money/authority modules it mutates IN PLACE (96 under
                         # apps/qr/lib, plus create-share-intent, setup-intent and board routes,
                         # plus EIGHT components — TableCartProvider.tsx, menu/YourUsual.tsx,
                         # staff/TicketText.tsx, staff/Chrome.tsx, staff/StaffPromoControl.tsx,
                         # ReadyBoard.tsx, MergeRedeemer.tsx and staff/TableCard.tsx, COMPONENTS in
                         # the set since M46 — plus
                         # packages/db/src/schemas.ts) and restores them.
                         # ⚠️ 96+3+8+1=108, and THIS ENUMERATION IS THE OPERATOR'S ONLY LIST of files
