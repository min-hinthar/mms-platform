...
pnpm verify:slice        # the MECHANICAL pre-PR gate: money-path coverage guard (a changed money
                         ...
                         # + photo-filter grep + gate + 836 mutations + orphan check (a few minutes)
                         # ⚠️ REWRITES the 146 money/authority modules it mutates IN PLACE (127 under
                         # apps/qr/lib, plus create-share-intent, setup-intent and board routes,
                         # plus FIFTEEN components — TableCartProvider.tsx, menu/YourUsual.tsx,
                         ...
                         # (Phase 2a), COMPONENTS in the set since M46 — plus
                         # packages/db/src/schemas.ts) and restores them.
                         # ⚠️ 127+3+15+1=146, and THIS ENUMERATION IS THE OPERATOR'S ONLY LIST of files
