...
- **The CI fast lane grew teeth** (T6 · M149). `format:check`, `check:migration-versions` and
  ...
  now FOURTEEN steps and the list above is only the first six** — measure it, never read it off this
  ...
  it lives in a component no suite and no `MONEY_PATHS` rule can see). ⚠️ `check:docs` is step ONE and GitHub's shell is
  `bash -e`, so a stale doc count is not cosmetic — it stops the THIRTEEN guards behind it, the orphan
...
pnpm verify:slice        # the MECHANICAL pre-PR gate: money-path coverage guard (a changed money
                         ...
                         # + photo-filter grep + gate + 741 mutations + orphan check (a few minutes)
                         # ⚠️ REWRITES the 127 money/authority modules it mutates IN PLACE (111 under
                         ...
                         # ⚠️ 111+3+12+1=127, and THIS ENUMERATION IS THE OPERATOR'S ONLY LIST of files
