...
pnpm verify:slice        # the MECHANICAL pre-PR gate: money-path coverage guard (a changed money
                         ...
                         # + photo-filter grep + gate + 657 mutations + orphan check (a few minutes)
                         # ⚠️ REWRITES the 117 money/authority modules it mutates IN PLACE (104 under
                         # apps/qr/lib, plus create-share-intent, setup-intent and board routes,
                         # plus NINE components — TableCartProvider.tsx, menu/YourUsual.tsx,
                         # staff/TicketText.tsx, staff/Chrome.tsx, staff/StaffPromoControl.tsx,
                         # ReadyBoard.tsx, MergeRedeemer.tsx, staff/TableCard.tsx and
                         # AccountUpgrade.tsx, COMPONENTS in the set since M46 — plus
                         # packages/db/src/schemas.ts) and restores them.
                         # ⚠️ 104+3+9+1=117, and THIS ENUMERATION IS THE OPERATOR'S ONLY LIST of files
