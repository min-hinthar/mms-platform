...
- **31 genuine adds** — 1,450 units of Jan–Jul 2026 volume, from Pork Tamarind Stew (151) down to
  Bottled Water (1). Each passed a machine check: no slug / English / exact-Burmese / loose-Burmese
  collision against the catalog, price **read from the named POS row, never transcribed**. Built as
...
- The reference backlog drops **98 → 60**, every residual row classified; §Price deltas stays
  "None". Migration `20260816040000` (idempotent; category joined by slug so a missing category
  fails loudly). Data-only change — no new money-path code, no new mutants needed.
