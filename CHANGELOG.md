...
- **`getGroceryLines(cartId)`** (`lib/grocery.ts`): the member-gated read behind the hydration — cart lines (`fulfillment='grocery'`, voided excluded, `created_at` order) joined with the catalog's presentation fields (`ebt_eligible`, `image_url`). Same `assertCartMember` gate as every cart read; read-only; hydration failures are a commented deliberate swallow (keep the last-known list; the next scan/focus re-syncs).
- **Product-grade rows:** photo (56px `BlurUpImage` on `grocery_items.image_url`, hidden when the catalog has none), name + EBT tag, `qty × unit` line, line total, and **qty steppers bound to CART-LINE ids through the existing `setQty` path** — no new money surface; qty 0 removes, matching the menu's rule. Steppers are 44px, `role="group"` with per-item aria-labels ("One more/less X", minus at qty 1 = "Remove X"), single-flight (`busyLine` disables the row's controls while an op is in flight), optimistic with a server reconcile after every write — a refused write (locked/settling/raced) snaps back to truth.
- **Honest empty state:** "Checking your basket…" until the first server read lands, then "Nothing scanned yet." — the page never claims an empty basket it hasn't verified.
- **K0 rides along:** `session_created` gains the analytics-only `door` property (`sessionMintInput.door` enum, never authz) — /grocery mints with `door: "grocery"`; the menu-page wiring lands with the K1 doors, and the door-keyed funnel re-pin waits for K6.
