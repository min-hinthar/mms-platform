...
- **"Scan & Go" retired diner-facing:** the menu eyebrow (`To-go`), the /track mode label (`To-go`), and the grocery H1 (`Scan your basket`) no longer carry the old brand name; staff surfaces (`FloorDetailLive`, `TableCard`) keep the internal `scango` vocabulary.
