...
- **W2** Finish the flagship — 🚧 _foundation shipped_ (this PR): ✅ designed missing-photo placeholder (`PhotoPlaceholder` + `BlurUpImage` fallback — never a broken tile) · ✅ SVG icon set retiring emoji chrome (`@mms/ui` `<Icon>` lucide set, diner path; staff deferred) · ✅ `loading.tsx` skeletons (menu/dine-in/track/grocery) + `error.tsx` chunk-reload guard · ✅ order code on the food /track receipt. **Remaining:** real photography (C5) + bucket → QR project · the ~360-`fontSize` type-scale sweep + lint ban · wallet-first Express Checkout + custom tip (W2d, money-path) · itemized + email receipt + print. ⬜
