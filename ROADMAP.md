...
- **P3.3b** Split-tender (**Option A**, confirmed by UX research) — each guest **authorizes** their own server-derived share (N `capture_method:manual` PaymentIntents); **capture-all + fulfill the one order when the last share clears**; **cancel auths on abandon/decline** (no one charged for an incomplete order); a `qr_cart_shares` ledger; share/line refunds. Weight tax by each seat's _taxable_ base + service by _net_ (not subtotal pro-rata). Pulls the **S4.3** seam forward per the milestone decision. ✅
