...
- **S1.1** Staff auth + **roles** (server / manager / owner), distinct from anon diners; RLS so staff read/write **any** table session, diners only their own. 🟡 _S1.1a shipped:_ magic-link/OTP auth + `staff` roles + `is_staff()` additive RLS (read-any) + owner provisioning at `/staff/team`. _S1.1b next:_ shared-tablet **PIN** fast-path.
