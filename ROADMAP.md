...
- **S2.1** Line-state machine **draft → fired → in-progress → served → settled** + KDS fire/bump; grocery lines lock at **payment**, not fire. 🟡 _**S2.1a shipped:** `qr_cart_items.state` column (backfilled `draft`) + atomic legal-edge RPC `mms_line_transition` (parent-cart-`open` guarded, service-role-only) + **`canMutateLine` v2** (staff a first-class actor; diner = own-draft-only; threaded through the diner server path so firing can't outpace the gate). Migration `20260622030000`. **S2.1b next:** KDS console view (fire queue + bump on `postgres_changes`) + dine-in immediate-fire + unify `fire_at` + grocery excluded._ &nbsp;Design: _Lives on `qr_cart_items` (PRE-settlement — the open cart is the table order until settle), not `qr_orders`. Transitions = atomic status-guarded RPC (`mms_line_transition`, legal-edge graph in SQL, parent-cart-`open` guard); **`canMutateLine` gains staff as a first-class actor** (post-fire = staff-only — the current `"host"` placeholder is a diner role, not staff). KDS v1 on `postgres_changes` (no broadcast); **unify the existing `fire_at`** (dine-in fires now, pickup `slot−prep`)._
