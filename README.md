...
pnpm verify:slice                      # the MECHANICAL money-path gate: coverage guard + 343 semantic
                                       # mutations (each MUST turn its owning suite red) + orphan check.
                                       # ⚠️ rewrites the 71 money/authority modules it mutates IN PLACE
                                       # (65 under apps/qr/lib, plus the create-share-intent, setup-intent
                                       # and board routes, plus TableCartProvider.tsx and menu/YourUsual.tsx —
                                       # the first COMPONENTS in the set, since M46) and restores them. It
                                       # ABORTS if a target file is DIRTY, so commit first.
