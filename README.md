...
pnpm verify:slice                      # the MECHANICAL money-path gate: coverage guard + 456 semantic
                                       # mutations (each MUST turn its owning suite red) + orphan check.
                                       # ⚠️ rewrites the 90 money/authority modules it mutates IN PLACE
                                       # (80 under apps/qr/lib, plus the create-share-intent, setup-intent
                                       ...
                                       # packages/db/src/schemas.ts: 80+3+6+1=90) and restores them.
