...
pnpm verify:slice                      # the MECHANICAL money-path gate: coverage guard + 408 semantic
                                       # mutations (each MUST turn its owning suite red) + orphan check.
                                       # ⚠️ rewrites the 83 money/authority modules it mutates IN PLACE
                                       # (74 under apps/qr/lib, plus the create-share-intent, setup-intent
                                       # and board routes, plus FIVE components — TableCartProvider.tsx,
                                       # menu/YourUsual.tsx, staff/TicketText.tsx, staff/Chrome.tsx and
                                       # staff/StaffPromoControl.tsx) and restores them. ⚠️ That list is
                                       # the operator's only record of what may be left broken on disk
                                       # by a killed run, so keep it measured, not counted by eye. It
