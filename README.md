...
**Build:** M0 · M2–M4 · S1–S4 · R1–R9 · J0–J6 ✅ (M1 🟡 — code done, owner-blocked infra tail) — shipped through **W22c** (the gesture layer), then the menu's first screen (**M131–M139**) and the Codex back-sweep arc (**#240–#242** — a money-path P1 from #233, the `codex-review` required check, the CI fast-lane teeth, and the four leftovers: dial-aware ambient motion, the split-`try` boot script, Rice off the promoted POS order, a themed reward shine) · **Gate:** 1102 qr tests + 138 ui tests · 228 `verify:slice` mutants · **Stack:** $0/mo software (Stripe per-txn only)
...
pnpm verify:slice                      # the MECHANICAL money-path gate: coverage guard + 228 semantic
                                       # mutations (each MUST turn its owning suite red) + orphan check.
                                       # ⚠️ rewrites the 57 money/authority modules it mutates IN PLACE
                                       # (54 under apps/qr/lib, plus the create-share-intent,
