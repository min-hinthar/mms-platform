...
pnpm verify:slice                      # the MECHANICAL money-path gate: coverage guard + 1117 semantic
                                       # mutations (each MUST turn its owning suite red) + orphan check.
                                       # ⚠️ rewrites the 163 money/authority modules it mutates IN PLACE
                                       # (138 under apps/qr/lib, plus the create-share-intent, setup-intent
                                       # and board routes, plus TWENTY-ONE components — TableCartProvider.tsx,
                                       # menu/YourUsual.tsx, staff/TicketText.tsx, staff/Chrome.tsx,
                                       # staff/StaffPromoControl.tsx, ReadyBoard.tsx,
                                       # MergeRedeemer.tsx, staff/TableCard.tsx, AccountUpgrade.tsx,
                                       # staff/SettledToday.tsx, staff/DayCash.tsx, Checkout.tsx,
                                       # PaymentSection.tsx, staff/CashSettleButton.tsx,
                                       # staff/CloseSecureTabButton.tsx, staff/TerminalSettle.tsx,
                                       # staff/FloorDetailLive.tsx, staff/OrderPad.tsx and three hooks
                                       # under components/ — staff/usePadWrites.ts, staff/useStaffSend.ts
                                       # and staff/usePadDetailLive.ts — plus
                                       # packages/db/src/schemas.ts: 138+3+21+1=163) and restores them.
...
| ------------------------------------------------------------------------ | --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`ci.yml`](.github/workflows/ci.yml)                                     | PR / push | **build** — a seconds-long fast lane FIRST (`check:docs` · `check:theme` · `check:types-sorted` · `format:check` · `check:migration-versions` · `check:promo-pin` — all file-read-only, no build, no DB) + orphan-suite guard + `turbo run lint typecheck build test` (cached) · **migrations-check + types-fresh** — boots the local Supabase stack, applies every migration + seed, proves the generated types aren't stale, runs the SQL tests (RLS · tax parity) |
...
| [`require-codex-review.yml`](.github/workflows/require-codex-review.yml) | PR        | pre-merge gate — RED until Codex has reviewed the PR's **current head**; publishes its verdict as a check run named **`codex-review`** created against `pr.head.sha`. Drafts stand down (mid-iteration). ⚠️ Not yet wired into branch protection — OPEN-ITEMS **C16**                                                                                                                                                                                                |

**The review is in-session, not in CI.** No workflow here runs a Claude review — the `review` / `security` / `adversarial-pr` stub checks that once existed only to satisfy branch protection have been **retired**, so the four workflows above are the whole of CI. The real gate, in order: `pnpm verify:slice` + `pnpm check:docs` (mechanical, zero tokens — `check:docs` also runs inside `ci.yml`, so a stale count fails the PR), then ONE fresh-context adversarial subagent over the diff (its verdict posted as a PR comment for the record), plus **two Codex rounds** — `@codex review` on the draft, round 2 on the fix commits — fix-or-justify both, then merge and file anything left in [`docs/OPEN-ITEMS.md`](docs/OPEN-ITEMS.md). **The Codex wait is MECHANICAL since #240**: `require-codex-review.yml` is red until Codex has reviewed the exact head about to merge, because the same sequencing failure shipped findings post-merge three times (#191, #239 by nine minutes, and a twelve-PR back-sweep that found a money P1 open for two days). It proves a review EXISTS on the merging commit; it can never prove anyone read it, so the fix-or-justify discipline above still stands — and until branch protection actually requires the `codex-review` check ([`docs/OPEN-ITEMS.md`](docs/OPEN-ITEMS.md) C16) it cannot even prove the wait happened: #241 was merged eleven seconds past it red, so the merge ritual in [`docs/WORKFLOW.md`](docs/WORKFLOW.md) (mark-ready and merge are never one motion; wait, event-driven, for green-on-head) is load-bearing. Calibration for the in-session pass lives in [`.github/claude-review-prompt.md`](.github/claude-review-prompt.md). **Quality:** ESLint + Prettier + knip via a shared `@mms/config` preset (`pnpm lint` / `pnpm format` / `pnpm knip`). **Claude Code** in this repo is configured by [`CLAUDE.md`](CLAUDE.md) + [`.claude/`](.claude) (settings, a post-edit auto-format hook, session-memory hooks, `LEARNINGS`/`ERROR_HISTORY`) and [`.mcp.json`](.mcp.json) (Supabase / GitHub / Sentry MCP). Day-to-day loop: [`docs/WORKFLOW.md`](docs/WORKFLOW.md). Contributing + templates: [`.github/`](.github).
