...
## #115 — An icon-only circle in the staff bar is named by sr-only `<Chrome>`, never by `sx()` (2026-09-15, A4·5)

Two guards refused the obvious `aria-label={sx(lang, key)}` on a bar circle, one after the other. `sx()`'s parameter type admits only `*.a11y.*` keys, so the first draft minted `kds.a11y.wall` / `browse.price.a11y.wordCheck` to satisfy tsc — and `check-staff-lang` rule 3 then refused `sx()` on an element that HAS visible children (the `<Icon>`): "that bypasses the {visible, aria} pair; use `al()`". The counter's approvals circle already shows the shape the guards want: a plain key (`floor.nav.approvals`) rendered as `<span className="sr-only"><Chrome lang={lang} k={key} /></span>` inside the link, no `aria-label` at all — the name is the device language, the glyph stays decorative, and no aria-only key exists to drift from a visible one. Reach for that shape first; an `.a11y.` key is for a name with NO rendered twin (a list, a region, a group).

## #116 — A migration directory has MANY definers of one function, and the answer can be added, dropped and restored; read the LAST one, then read prod (2026-09-17, M217)

A high-severity row sat open for four days describing a defect that had been fixed months before it was filed. **Every number in the first draft of this entry was wrong, which is the lesson twice over** — the blind adversarial pass on #287 falsified all three, and they are corrected here from measurement:

- `mms_merge_table_orders` is redefined by **14** migration files (`grep -lE "create or replace function[[:space:]]+(public\.)?mms_merge_table_orders" supabase/migrations/*.sql | wc -l`). A bare `grep -l` on the name returns **15** — it also hits `20260823000000_m100_mode_authority.sql`, which mentions the function without redefining it. Mentions are not definitions, and the first draft conflated them in the same sentence that prescribed `grep -l` as the discovery step.
- The shell sorts a glob lexicographically, so the **first** hit is `20260621160000_table_merge.sql`. The first draft claimed it was `20260623030000_s3_secure_merge_guard.sql`, which is simply not what that command prints.
- The S6 same-kitchen-state fold (`and t.state = r.state`) was **not** added by `20260822000000_m98…`, whose own in-body comment says what it added (`-- M98: …and the same PRICE.`). Measured across the 14 definers in order, the fold is present in exactly 7, and its history is **add → drop → restore**: introduced `20260622090000_s2_audit_fixes.sql`, **absent again from the next three definers** (`20260623010000`, `20260623030000`, `20260629120000`), restored by `20260702000000_merge_void_guard_restore.sql`, and carried by every definer since.

That drop-and-restore is what makes the rule necessary rather than merely tidy. The reviewer who filed M217 read `20260623030000` — a file in which the defect was **genuinely live** — and described it accurately. Two of its neighbours agree with it. Only the LAST definer answers the question, and only prod answers whether the repo's last definer is what actually runs (this repo's prod history shares zero version stamps with the filenames — CLAUDE.md).

I then nearly shipped the same mistake one step further in the same session: a full replacement function and a nine-case SQL test, both written against that superseded body, which would have reverted five migrations' worth of guards in one commit. Caught by noticing `m97_`/`m98_` tests already registered in `ci.yml`; both files were deleted before they were committed.

So, for anything defined in `supabase/migrations/`: `grep -lE "create or replace function …"` for the DEFINERS (not the mentions), `tail -1` for the current one, read THAT, then settle it with `pg_get_functiondef` over the MCP. And when you write the history down, measure every count in the shell and paste the output — a narrative about migrations is exactly the kind of prose that reads as authoritative and is checked by nobody.

## #117 — A defect row is closed against a SCREEN; the rule was about a HOOK, and the second consumer kept shipping it (2026-09-17, M193)

M193 was "each cart mutation costs three `getCartView` chains". #275 built a coalescer in `TableCartProvider`, proved it with four tests and a mutant, and closed the row. `useCartRealtime` has **two** consumers, and the other one — `Checkout.tsx`, the pre-payment screen, where the cost is worst — was untouched: an arrow that ignored its `CartChange` and called `refresh()` per event, the row's text verbatim. /cart is its own route with its own tree, so the provider's coalescer is not mounted there and NO test of the provider could ever have gone red for it.

The tell is grammatical. When a finding names a mechanism (`useCartRealtime` calls `onChange` per row event) rather than a place, the subject is the mechanism's CONSUMER SET, and a fix at one consumer closes one instance. Before closing such a row: `grep -rn "<theMechanism>"`, count the non-test consumers, and fix or explicitly account for each.
...
1. **A matcher keyed on an exported NAME is defeated by the import.** Collecting `CallExpression`s whose callee is the identifier `useCartRealtime` misses `import { useCartRealtime as useRealtime }` entirely — and a FLOOR does not rescue you, because the existing correct call sites still satisfy it. Resolve the local BINDING from the import (`propertyName ?? name`, plus `import * as ns` → `ns.export`).
2. **A hand-written set of module-specifier spellings is the same bug one level up.** `["@/lib/x", "./x", "../lib/x"]` means a fourth spelling is simply not the module. Resolve the specifier to a repo-relative PATH.
3. **`statements.some(isTheCall)` asserts presence, not reachability.** `if (irrelevant) return; schedule();` satisfies it while whole classes of input skip the behaviour. Walk in order and refuse once a preceding statement can `return`/`throw` out of the callback — and **do not descend into nested function-like nodes**, or you refuse correct code (an arrow's own `return` inside a preceding `.find(…)` exits the arrow, not the handler).
