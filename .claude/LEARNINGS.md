...
## #116 — `grep` returns the OLDEST definition first, and a migration directory has many; read the LAST definer, then read prod (2026-09-17, M217)

A high-severity row sat open for four days describing a defect that had been fixed a month before it was filed. `mms_merge_table_orders` is redefined by **eleven** migration files; `grep -l mms_merge_table_orders supabase/migrations/*.sql` lists them in filename order, so the first hit is the OLDEST — `20260623030000_s3_secure_merge_guard.sql`, whose body genuinely does loop with no state predicate. The reviewer read that body, described it accurately, and filed a finding against a database that has not existed since June: `20260822000000_m98_merge_matches_price.sql` added the S6 same-kitchen-state fold (`and t.state = r.state`) and `20260827000000_m109_merge_matches_mode.sql` carried it forward.

I made the same mistake one step further in the same session — I wrote a full replacement function and a nine-case SQL test against that superseded body before noticing `m97_`/`m98_` tests already registered in `ci.yml`. Both files were deleted; had they merged they would have REVERTED five migrations' worth of guards in one commit.

So, for anything defined in `supabase/migrations/`: `grep -l` to find the candidates, then `tail -1` to find the LAST definer, and read THAT. And because this repo's prod history is divergent (CLAUDE.md), the repo's last definer is still only a hypothesis about prod — settle it with `pg_get_functiondef` over the MCP, which is what finally closed M217. Two minutes of `pg_get_functiondef` beats a migration written against a fiction.
...
## #118 — Two ways a PARSED guard still fails to establish its property: the NAME it matches, and `some()` where it means "reached" (2026-09-17, #287 Codex round 1)

`check-echo-coalesce.mjs` was written to the #60 rules — it parses, it resolves declarations, it refuses conditionals — and Codex still found two holes in it, with zero findings in the product code beside it. Neither is a scan; both are worth naming.

**1. A matcher keyed on an exported name is defeated by the IMPORT.** The guard collected `CallExpression`s whose callee is the identifier `useCartRealtime`. A new consumer writing `import { useCartRealtime as useRealtime }` calls `useRealtime(...)`, which the guard does not see at all — and the FLOOR does not save you, because the two existing unaliased sites still satisfy `MIN_CALL_SITES`. The guard would have printed its success line over exactly the defect it exists to prevent. Resolve the local BINDING from the import instead (named alias via `propertyName ?? name`, plus `import * as ns` → `ns.export`), and resolve the module SPECIFIER as a path (`@/…` and any relative depth) rather than against a hand-written set of spellings — the set-of-spellings mistake in a second costume.

**2. `body.statements.some(isTheCall)` asserts presence, not reachability.** `if (irrelevant) return; schedule();` satisfies it: the call really is a top-level `ExpressionStatement`, and whole classes of event still skip the refresh. Position is not the fix either. The property is "no statement before the call can transfer control out of the callback", so walk each preceding statement for `return`/`throw` and **do not descend into nested function-like nodes** — an arrow's `return` inside a preceding `seats.find((s) => { return …; })` exits the arrow, not the handler, and treating it as an exit refuses correct code.

The generalisation, to run after writing any parsed guard: ask separately _"what NAME can the subject wear that I do not match?"_ and _"what CONTROL FLOW satisfies my structural test while skipping the behaviour?"_. Falsify both — and hold a GREEN control for each tightening, or you trade a false negative for a false positive.
