 ...
 *   - §2: the four pressed selectors on the console share ONE rule — a second copy is the drift
...
describe("§2 — the console's four pressed selectors share ONE lit-cap rule", () => {
  const PRESSED = [
  ...
  it("declares a fill for each of them in exactly one block, and it is the same block", () => {
    ...
    expect(bodies.size, "all four pressed selectors resolve to one declaration").toBe(1);
