...
describe("liveOrderStatusWord", () => {
  ...
  it("reads dine-in neutrally — a dine-in order has no bagging signal, ever", () => {
    expect(liveOrderStatusWord({ kind: "dinein", togoStatus: null })).toBe("At your table");
  });
  ...
  it("lets a to-go box on a dine-in order win the word (the shipped tie rule)", () => {
    ...
    expect(liveOrderStatusWord({ kind: "dinein", togoStatus: "preparing" })).toBe("Preparing");
    expect(liveOrderStatusWord({ kind: "dinein", togoStatus: "ready" })).toBe("Ready");
  });

  it("distinguishes pickup from to-go only when the bag is ready", () => {
    expect(liveOrderStatusWord({ kind: "pickup", togoStatus: "ready" })).toBe("Ready for pickup");
    expect(liveOrderStatusWord({ kind: "togo", togoStatus: "ready" })).toBe("Ready");
  });

  it("says 'Order received' — not 'Preparing' — before the kitchen has touched it", () => {
    expect(liveOrderStatusWord({ kind: "togo", togoStatus: null })).toBe("Order received");
    expect(liveOrderStatusWord({ kind: "pickup", togoStatus: null })).toBe("Order received");
  });
  ...
  it("is TOTAL over the column's CHECK — picked_up included, so /track can share it", () => {
    ...
    expect(liveOrderStatusWord({ kind: "togo", togoStatus: "picked_up" })).toBe("Picked up");
    expect(liveOrderStatusWord({ kind: "pickup", togoStatus: "picked_up" })).toBe("Picked up");
  });
});
