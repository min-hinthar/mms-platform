 ...
 * code beside its English sentence, so the board can say it in the device language. `gate` keeps
 * the gate's own copy (the write-outage twin lives in `OutageText`; the role floor has no Burmese
 * yet, and a sentence we cannot translate is shown in English rather than guessed at). `signin` is
 * not a message at all — the board leaves for /staff/login, the honest surface (K10).
 */
