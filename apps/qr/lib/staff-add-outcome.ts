import { ItemUnreadableError, ItemUnsellableError } from "./order-lines";
 ...
 * `unconfirmed` whatever it says.
 */

/** A failure `addFailureCode` can decide from the phase + the thrown value. */
export type AddFailureCode = "sold_out" | "gone" | "outage" | "failed" | "unconfirmed";
 ...
 *   • `closed`   — the table is closed; `no-cart` — it has no open order;
...
export function addFailureCode(phase: "price" | "write", err: unknown): AddFailureCode {
  // The write may have committed — the one outcome a caller must never retry under a new key.
