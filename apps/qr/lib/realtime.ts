 ...
 */
export function useGroupCart(sessionId: string, accessToken: string, me: { seat: string; name: string }) {
  ...
  useEffect(() => {
    ...
    const channel = supa.channel(`table:${sessionId}`, { config: { private: true, presence: { key: sessionId } } });
    chanRef.current = channel;

    channel
      .on("presence", { event: "sync" }, () => {
        const state = channel.presenceState<{ seat: string; name: string }>();
        setMembers(Object.values(state).flat().map((p) => ({ seat: p.seat, name: p.name })));
      })
      ...
      .subscribe(async (status) => {
        if (status === "SUBSCRIBED") {
          await channel.track(me); // stable seat/name from the session JWT — no per-subscribe presence churn
        }
      });

    return () => { supa.removeChannel(channel); };
  }, [sessionId, accessToken, me.seat, me.name]);
