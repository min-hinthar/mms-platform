 ...
 * caller themself, the CALLER is within the step-up rate bucket, and the approver id resolves to an
 * ACTIVE manager/owner. The RPCs re-check role + self atomically — this just stops the DoS upstream.
 */
export async function approverStepUpAllowed(
  approverStaffId: string,
  callerStaffId: string,
): Promise<"ok" | "bad_approver" | "step_up_rate_limited"> {
  if (approverStaffId === callerStaffId) return "bad_approver";
