 ...
 *    several, but the read is `mms_usual_lines(uid, since)` — a `SECURITY DEFINER` function whose
 *    WHERE clause pins every row to that uid, revoked from `public`/`anon`/`authenticated` so it can
 *    never be reached from a browser. The only thing that LEAVES is a menu item id and a name the
 *    diner can already see on the menu. No amounts, no order ids, no dates.
 ...
 * door, which carries eight. Since **M87** the ATTRIBUTION rule is not here either — it is the WHERE
 * clause of `mms_usual_lines`, pinned by `supabase/tests/m87_order_item_seat_test.sql` against a real
 * database and driven by the real fulfill RPCs. What is left in this file is a call and a degradation
 * path, and its one security-critical line — passing the caller's own uid and never an argument — is
...
/**
 * One diner's 90 days is small, and `mms_usual_lines` already scopes to that window and that uid, so
 * there is no row cap here any more. The pre-M87 query carried one because it ordered and truncated
 * client-side; the function returns only rows that are already this person's.
 */
export async function getYourUsual(catalog: UsualCandidate[]): Promise<UsualOutcome> {
  try {
    ...
    for (const r of data ?? []) {
      ...
      rows.push({ menuItemId: id, orderId: r.order_id, orderedAt: r.ordered_at });
    }
