 ...
 *      clientSecret — Elements are re-created only by a LOAD retry before the form is revealed (a
 *      load failure, or the online auto-retry of a load that saw offline) or by a new clientSecret,
 *      never after a decline, which needs a revealed form that nothing un-reveals — and returns the
 *      diner to a live Pay button, so
