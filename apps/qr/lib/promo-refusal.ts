 ...
 */
export type PromoRefusal = "error" | "cart_closed" | "locked" | "code_applied";

/**
 * `attempted` — the code the caller was trying to WRITE, upper-cased, when the write was an apply.
 *
 * The staff apply carries `promo_code.is.null,promo_code.eq.<attempted>` so it can never silently
 * replace a code a guest was already quoted (see `staff-promo.ts`). That predicate is invisible from
 * here: a cart that is open, unfrozen and carries a DIFFERENT code matches nothing and would fall
 * through to the `cart_closed` floor — a verdict about a cart that is demonstrably open, which is the
 * exact M116/M119 fabricated-diagnosis shape. Passing the attempted code is what lets the diagnosis
 * see the predicate that refused it. Omitted (a remove, or the diner path) it changes nothing — and
 * the overloads say so in the TYPE: without `attempted` this cannot return `code_applied`, so
 * `applyPromo`'s narrower `PromoReason` union stays sound without a cast or a dead branch.
 */
export async function refusedPromoReason(
  cartId: string,
): Promise<Exclude<PromoRefusal, "code_applied">>;
export async function refusedPromoReason(
  cartId: string,
  opts: { attempted: string },
...
export async function refusedPromoReason(
  ...
  const { data: cart, error } = await serviceClient()
    .from("qr_carts")
    .select("status,locked,locked_at,settle_at,live_payment_intent_id,promo_code")
    .eq("id", cartId)
    .maybeSingle();
  ...
  // AFTER the freeze branches on purpose: while money is moving, "someone's paying" is the more
  // urgent truth and removing the other code would not help anyway. Unfrozen, a code already on the
  // cart is the whole reason the write matched nothing, and it is ACTIONABLE — the remove is one tap
  // away on the same control.
  if (opts?.attempted && cart.promo_code && cart.promo_code !== opts.attempted)
    return "code_applied";
